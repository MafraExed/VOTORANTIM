sap.ui.define([
	...
	'sap/m/MessageBox',
	'zvpwmm0001_rc/model/formatter'
], function (jQuery, Controller, Filter, History, JSONModel, MessageBox, formatter) {
	"use strict";

	return Controller.extend("zvpwmm0001_rc.controller.Default", {
		formatter: formatter,
		onInit: function () {
		...
		onItemSelect: function (oEvent) {
			...
			var Documento = oEvent.getSource().getBindingContext().getProperty("Banfn");
			var Item = oEvent.getSource().getBindingContext().getProperty("Bnfpo");
			var Kostl = oEvent.getSource().getBindingContext().getProperty("Kostl");
			if (Kostl.length == '0') {
				Kostl = "Default";
			}
			oRouter.navTo("page", {
		...
		onApproveLote: function (oEvent) {
			...
			var oTable = this.getView().byId("IdHeaderSet");
			var oModel = oTable.getModel();
			var aItems = oTable.getSelectedItems(); //"getItems();
			...
			if (aItems.length === 0) {
				...
				if (!this._dialog) {
					var vBusyDialog = sap.ui.xmlfragment("zvpwmm0001_rc.fragment.BusyDialog", this);
				...
				for (var iRowIndex = 0; iRowIndex < aItems.length; iRowIndex++) {
					var l_Banfn = oModel.getProperty("Banfn", aItems[iRowIndex].getBindingContext());
					var l_Bnfpo = oModel.getProperty("Bnfpo", aItems[iRowIndex].getBindingContext());
				...
				oDataModel.create("/Requisicaos", oEntry, {
					success: function (oData, oResp) {
						setTimeout(function () {
							...
							sap.m.MessageToast.show(oTitleDocsProcessMass, {
								...
								width: "15em", // default
								my: "center", // default center bottom
								at: "center", // default center bottom
								of: window, // default
								offset: "0 0", // default
		...
		onApprove: function (oEvent) {
			...
			if (rows.length === 0) {
				...
				vBusyDialog.open();
				var oModel = new sap.ui.model.odata.ODataModel("/sap/opu/odata/sap/ZGWFBRC_PORTAL_SRV/");
				oModel.setHeaders({
				...
				for (var i = 0; i < rows.length; i++) {
					...
					item = (array[2].substring(0, 5));
					oEntry = ({
						Banfn: documentos,
						Bnfpo: item
					});
					batchChanges.push(oModel.createBatchOperation("Requisicaos", "POST", oEntry));
				}
