sap.ui.define([
	...
	"use strict";
	var _timeout;
	return Controller.extend("zvpwmm0001_rc.controller.Page", {
		...
		createPopover: function() {
			if (!this._oQuickView) {
				this._oQuickView = sap.ui.xmlfragment("zvpwmm0001_rc.fragment.Grupo", this);
		...
		onRejeitar: function(oEvent) {
			if (!this._dialog) {
				var vBusyDialog = sap.ui.xmlfragment("zvpwmm0001_rc.fragment.BusyDialogRej", this);
				this.getView().addDependent(vBusyDialog);
			}

			var oModelP = this.getView("Default").getModel();
			var IdDocumento = oEvent.getSource().getBindingContext().getProperty("Banfn");
			var IdItem = oEvent.getSource().getBindingContext().getProperty("Bnfpo");
		...
		_onRouteMatched: function(oEvent) {
			...
			oView = this.getView();
			oView.bindElement({
				path: "/Requisicaos(Banfn='" + oArgs.Documento + "',Bnfpo='" + oArgs.Item + "',Kostl='" + oArgs.Kostl + "')",
				//path: "/Requisicaos(Banfn='" + oArgs.Documento + "',Bnfpo='" + oArgs.Item + "')",
				events: {
		...
		handleResponsivePopoverPress: function(oEvent) {
			...
			//	if (!this._oPopover) {
			var oView = this.getView();
			this._oPopover = sap.ui.xmlfragment(oView.getId(), "zvpwmm0001_rc.fragment.Grupo", this);
		...
		onMessagePopOver: function(oArgs) {
			var that = this;
			var oModel = this.getView().getModel();
			var sPath = ("/Requisicaos(Banfn='" + oArgs.Documento + "',Bnfpo='" + oArgs.Item + "',Kostl='" + oArgs.Kostl + "')/ToLog");
		...
		onHistory: function(oEvent) {
			...
			var oView = this.getView();
			this._oPopover = sap.ui.xmlfragment(oView.getId(), "zvpwmm0001_rc.fragment.History", this);
			this.getView().addDependent(this._oPopover);
			this._oPopover.openBy(oEvent.getSource());
