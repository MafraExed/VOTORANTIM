sap.ui.define([
	...
	return BaseController.extend("Y5GL_BEM_ESTAR.Y5GL_BEM_ESTAR.controller.BENEFICIOS_DETAIL", {
		...
		onInit: function () {
			...
			// Image loading
			var sRootPath = jQuery.sap.getModulePath("Y5GL_BEM_ESTAR.Y5GL_BEM_ESTAR");
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getView().byId("idChamado").setValue(chamado);
		...
		onSave: function (via2) {
			...
			if (Beneficio == "EMP_CONSIGNADO" ||
                Beneficio == "LABORAL" ||
                Beneficio == "PASAJE" ||
                Beneficio == "ADELANTO" ||
                Beneficio == "ADELANTO_SUELDOS" || 
                Beneficio == "ASIGNACION" ||
				Beneficio == "SEGURO_DE_VIDA") {
				Tipo = 'L';
			} else if (Beneficio == "PLANO_MEDICO") {
				Tipo = "K";
			} else if (Beneficio == "ASIGNACION_ESCOLAR" || Beneficio == "AGUINALDO_NAVIDENO") {
                Tipo = "A";
            }
			...
			switch (Beneficio) {
			case "EMP_CONSIGNADO":
				//oEntry.Rubrica = this.getView().byId("IdPlano_EMP_CONSIGNADO").getValue();
				desativaValor = "x";
				break;
            case "ADELANTO":
                desativaValor = "x";
			...
			case "SEGURO_DE_VIDA":
				...
				var risco = this.getView().byId("IdOpcoes_risco").getSelectedKey();
				if (!risco) {
					sap.m.MessageBox.error("Seleccione el riesgo");
					return;
				}
				oEntry.Valor = this.getView().byId("IdOpcoes_risco").getSelectedKey();
				desativaValor = "x";
				break;
			case "PLANO_MEDICO":
		...
		GravaBeneficio: function (key, oEntry, via2) {
			...
			if (via2 === "X") {
				menssagemsave = "¿Confirmar aprobación duplicada?";
			} else {
				menssagemsave = "¿Confirma la aprobación del beneficio?";
			}
					...
					press: function () {
						oModel.update(key, oEntry, {
							success: function (oData, oResponse) {
								sap.m.MessageBox.success("Beneficio aprobado con éxito.", {
									actions: ["OK", sap.m.MessageBox.Action.CLOSE],
									onClose: function (sAction) {
										that.getView().getModel().refresh(true);
		...
		_onBindingChange: function () {
			...
			if (param !== undefined) {
				...
				case "ESTAC_MOVBUS":
					...
					var oFiltercham = new sap.ui.model.Filter("chamado", sap.ui.model.FilterOperator.EQ, chamado);
					
					var oList = this.getView().byId("UploadCollection");
					oList.getBinding("items").filter([oFilterPernr, oFilterInfty, oFilterSubty, oFiltercham, oFilterTipo, oFilterValor]);
					return;
				...
				case "PASAJE":
					...
					this.getView().byId("IdPlano_PASAJE").setSelectedKey(mes1);
					this.getView().byId("IdAno_PASAJE").setSelectedKey(ano1);
					infty = "XXXX";
					subty = "PASA";
				...
				case "SEGURO_DE_VIDA":
					...
					var oList = this.getView().byId("UploadCollection");
					oList.getBinding("items").filter([oFilterPernr, oFilterInfty, oFilterSubty, oFiltercham, oFilterTipo]);
					return;
		...
		onEdit: function () {
			...
			case "EMP_CONSIGINADO":
				...
				break;
            case "ADELANTO":
                visao.byId("IdPlano_ADELANTO").setEditable(true);
                visao.byId("IdMontante_ADELANTO").setEditable(true);
                visao.byId("IdValidade_ADELANTO").setEditable(true);
                visao.byId("IdObservacao_ADELANTO").setEditable(true);
                break;
            case "ADELANTO_SUELDOS":
                visao.byId("IdPlano_ADELANTO_SUELDOS").setEditable(true);
                visao.byId("IdMontante_ADELANTO_SUELDOS").setEditable(true);
                visao.byId("IdValidade_ADELANTO_SUELDOS").setEditable(true);
                visao.byId("IdObservacao_ADELANTO_SUELDOS").setEditable(true);
                break;
            case "ASIGNACION":
                visao.byId("IdPlano_ASIGNACION").setEditable(true);
                visao.byId("IdMontante_ASIGNACION").setEditable(true);
                visao.byId("IdValidade_ASIGNACION").setEditable(true);
                visao.byId("IdObservacao_ASIGNACION").setEditable(true);
                break;
            case "ASIGNACION_ESCOLAR":
                visao.byId("IdPlano_ASIGNACION_ESCOLAR").setEditable(true);
                visao.byId("IdMontante_ASIGNACION_ESCOLAR").setEditable(true);
                visao.byId("IdValidade_ASIGNACION_ESCOLAR").setEditable(true);
                visao.byId("IdObservacao_ASIGNACION_ESCOLAR").setEditable(true);
                break;
            case "AGUINALDO_NAVIDENO":
                visao.byId("IdPlano_AGUINALDO_NAVIDENO").setEditable(true);
                visao.byId("IdMontante_AGUINALDO_NAVIDENO").setEditable(true);
                visao.byId("IdValidade_AGUINALDO_NAVIDENO").setEditable(true);
                visao.byId("IdObservacao_AGUINALDO_NAVIDENO").setEditable(true);
                break;
			case "FARMACIA":
		...
		formatUploadCollectionLabel: function (sBeneficio) {
			if (sBeneficio === "ADELANTO" ||
				sBeneficio === "ADELANTO_SUELDOS" ||
				sBeneficio === "ASIGNACION" ||
				sBeneficio === "PASAJE_UNIVERSITARIO" ||
				sBeneficio === "ASIGNACION_ESCOLAR" ||
				sBeneficio === "AGUINALDO_NAVIDENO" ||
				sBeneficio === "EMP_CONSIGNADO" ||
				sBeneficio === "ESTAC_MOVBUS" ||
				sBeneficio === "SEGURO_DE_VIDA") {
				return null;
			} else {
				return "Para agregar / cambiar archivos adjuntos, seleccione el botón (+)";
			}
		}
	});
