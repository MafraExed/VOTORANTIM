sap.ui.define([
	...
	return BaseController.extend("Y5GL_BEM_ESTAR.Y5GL_BEM_ESTAR.controller.DADOS_PESSOAIS", {
		onInit: function () {
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getView().byId("idTitleDependentes").setText("Dados Pessoais - (Matrícula - " + Pernr + ") - Chamado - " + Chamado + " ");
		...
		onAprovar: function () {
			...
			key = "/ZET_MEUS_DADOS_ECSet(Pernr='0',Tipo='G')";
					...
					press: function () {
						oModel.update(key, oEntry, {
							success: function (oData, oResponse) {
								...
								sap.m.MessageBox.success("Sua inclusão/alteração foi iniciada e seguirá para processamento. Aguarde retorno ou acompanhe pelo app.", {
		...
		handleValueHelp: function (oEvent) {
			...
			if (!this._valueHelpDialog1) {
				this._valueHelpDialog1 = sap.ui.xmlfragment("Y5GL_BEM_ESTAR.Y5GL_BEM_ESTAR.view.LocNascimento", this);
		...
		onuploadComplete: function (oEvent) {
			...
			if (Pernr !== "" || Subty !== "") {
				...
				oList.getBinding("items").filter([oFilterInfty, oFilterSubty, oFilterTipo, oFiltercham]);
			}
		},
		...
		onmodelListContextChange: function (oEvent) {
			...
			if (Pernr !== "" || Subty !== "") {
				...
				var oFiltercham = new sap.ui.model.Filter("chamado", sap.ui.model.FilterOperator.EQ, Chamado);
				var oList = this.getView().byId("UploadCollection");
				oList.getBinding("items").filter([oFilterPernr, oFilterInfty, oFilterSubty, oFilterTipo, oFiltercham]);
			}
		},
