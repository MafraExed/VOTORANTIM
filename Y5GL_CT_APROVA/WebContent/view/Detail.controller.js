/*global location */
sap.ui.define([
	"vsa/y5gl_ct_portal/view/BaseController",
	"sap/ui/model/json/JSONModel",
	"vsa/y5gl_ct_portal/model/formatter",
	"sap/ui/model/FilterOperator",
	"sap/ui/model/Filter"
], function (BaseController, JSONModel, formatter, FilterOperator, Filter) {
	"use strict";

	return BaseController.extend("vsa.y5gl_ct_portal.view.Detail", {
		...
		onInit: function () {
			...
			this.getRouter().getRoute("sCt").attachPatternMatched(this._onObjectMatched, this);
		...
		_onObjectMatched: function (oEvent) {
				...
				this.getModel().metadataLoaded().then(function () {
					var sObjectPathHeader = this.getModel().createKey("Contratos", {
		...
		onApprove: function (e) {			
			if (!this._oDialogApprove) {
				this._oDialogApprove = sap.ui.xmlfragment("vsa.y5gl_ct_portal.view.fragment.approve", this);
		...
		onApprove_W: function (e) {			
			if (!this._oDialogApprove_W) {
				this._oDialogApprove_W = sap.ui.xmlfragment("vsa.y5gl_ct_portal.view.fragment.approve_w", this);
		...
		onReject: function (e) {						
			...
			aFilters.push(new Filter("Processo", FilterOperator.EQ, "CT"));
		...
		_openRejectionPopup: function(){
			if (!this._oDialogReject) {
				this._oDialogReject = sap.ui.xmlfragment(this.getView().getId(),"vsa.y5gl_ct_portal.view.fragment.reject", this);
		...
		onQuestion : function(e){
			...
			if (!this._oDialogQuestion) {
				this._oDialogQuestion = sap.ui.xmlfragment("vsa.y5gl_ct_portal.view.fragment.question", this);
		...
		onInterruption : function(e){
			...
			if (!this._oDialogInterruption) {
				this._oDialogInterruption = sap.ui.xmlfragment("vsa.y5gl_ct_portal.view.fragment.interrupt", this);
