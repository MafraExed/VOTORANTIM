sap.ui.define(["Y5GL_DHO4/Y5GL_DHO4/controller/BaseController", "sap/ui/Device", "sap/m/Dialog", "sap/m/Button", "sap/m/Text",
	...
	function (BaseController, Device, Dialog, Button, Text, Filter, Label, TextArea, ButtonType) {
		...
		var param, edit, dep, chaveacao, via2, Cancela, Pernr, chamado, valorFiltro;
		var operacao = "";

		return BaseController.extend("Y5GL_DHO4.Y5GL_DHO4.controller.BENEFICIOS_DETAIL", {
			...
			onInit: function () {
				...
				// Image loading
				var sRootPath = jQuery.sap.getModulePath("Y5GL_DHO4.Y5GL_DHO4");
			...
			onSave: function (via2) {
				...
				if (Beneficio == "EMP_CONSIGNADO" ||
					...
					Tipo = 'K';
				} else if (Beneficio == "REEMBOLSO_IDIOMA" || 
                    Beneficio == "PASAJE" || 
                    Beneficio == "ESTAC_MOVBUS" ||
                    Beneficio == "PASAJE_UNIVERSITARIO") {
					Tipo = "A";
				}
			...
			onSaveCDep: function () {
				...
				if (operacao === "I") {
					menssagem = "¿Confirmas la inclusión del dependiente en el plan?";
					Concatenate = "I@";
				} else if (operacao === "C") {
					menssagem = "¿Confirma la solicitud duplicada?";
					Concatenate = "C@";
				} else if (via2 === "X") {
					Concatenate = "2@";
				} else {
					menssagem = "¿Confirma la exclusión del dependiente del plan?";
					Concatenate = "E@";
				}
				...
				if (via2 === "X") {
					dialog = new Dialog({
						...
						type: "Message",
						content: new Text({
							text: "¿Confirma la solicitud duplicada para dependientes seleccionados?"
						}),
						beginButton: new Button({
							text: "Sí",
							press: function () {
			...
			filtraUploadCollection: function () {
				var infty, subty, buscaValor, valor;
				...
				if (param !== undefined) {
                    ...
                    if (buscaValor !== "x") {
                        valor = this.getView().byId("idValor").getValue();
                    } else {
                        valor = "";
                    }
			...
			estacMovBus: function () {
				...
				valorFiltro = this.getView().byId("IdPlano_ESTAC_MOVBUS").getValue();
				valor = valorFiltro.split("_");
				mes = valor[0];
				ano = valor[1];

				this.getView().byId("IdPlano_ESTAC_MOVBUS").setSelectedKey(mes);
				this.getView().byId("IdAno_ESTAC_MOVBUS").setSelectedKey(ano);
			},
			laboral: function () {
				var valor, mes, ano;

				valorFiltro = this.getView().byId("IdPlano_LABORAL").getValue();
				valor = valorFiltro.split("_");
				mes = valor[0];
				ano = valor[1];

				this.getView().byId("IdPlano_LABORAL").setSelectedKey(mes);
				this.getView().byId("IdAno_LABORAL").setSelectedKey(ano);
			},
			pasaje: function () {
				var valor, mes, ano;

				valorFiltro = this.getView().byId("IdPlano_PASAJE").getValue();
				valor = valorFiltro.split("_");
				mes = valor[0];
				ano = valor[1];

				this.getView().byId("IdPlano_PASAJE").setSelectedKey(mes);
				this.getView().byId("IdAno_PASAJE").setSelectedKey(ano);
			},
			reembolsoCursos: function () {
				var valor, mes, ano;

				valorFiltro = this.getView().byId("IdPlano_REEMBOLSO_CURSOS").getValue();
				valor = valorFiltro.split("_");
				mes = valor[0];
				ano = valor[1];

				this.getView().byId("IdPlano_REEMBOLSO_CURSOS").setSelectedKey(mes);
				this.getView().byId("IdAno_REEMBOLSO_CURSOS").setSelectedKey(ano);
			},
			pasajeUniversitario: function () {
                var valor, mes, ano;

                valorFiltro = this.getView().byId("idValor").getValue();
                valor = valorFiltro.split("_");
                mes = valor[0];
                ano = valor[1];
    
                this.getView().byId("IdPlano_PASAJE_UNIVERSITARIO").setSelectedKey(mes);
                this.getView().byId("IdAno_PASAJE_UNIVERSITARIO").setSelectedKey(ano);
			...
			reembolsoExpatriado: function () {
				...
				valorFiltro = this.getView().byId("IdPlano_REEMBOLSO_EXPATRIADO").getValue();
				valor = valorFiltro.split("_");
				mes = valor[0];
				ano = valor[1];

				this.getView().byId("IdPlano_REEMBOLSO_EXPATRIADO").setSelectedKey(mes);
				this.getView().byId("IdAno_REEMBOLSO_EXPATRIADO").setSelectedKey(ano);
			},
			reembolsoIdioma: function () {
				var valor, mes, ano;

				valorFiltro = this.getView().byId("IdAno_REEMBOLSO_IDIOMA").getValue();
				valor = valorFiltro.split("_");
				mes = valor[0];
				ano = valor[1];

				this.getView().byId("IdPlano_REEMBOLSO_IDIOMA").setValue(mes);
				this.getView().byId("IdAno_REEMBOLSO_IDIOMA").setValue(ano);
			},
			seguroVida: function () {
                var valor = this.getView().byId("idValor").getValue();

                this.getView().byId("IdOpcoes_risco").setSelectedKey(valor);
