		...
		onTheMasterPage: {
			...
			actions: {
				...
				iSortTheListOnName: function () {
					return this.iChooseASorter("sortButton", "Sort By <Pernr>");
				},
				iSortTheListOnUnitNumber: function () {
					return this.iChooseASorter("sortButton", "Sort By <Index>");
				},

				iFilterTheListOnUnitNumber: function () {
					return this.iChooseASorter("filterButton", "<Index>", "<100 ");
				},

				iGroupTheList: function () {
					return this.iChooseASorter("groupButton", "<Index> Group");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("ZET_GLHR_PERIODOS");
							var aObjectsNotInTheList = aEntityData.filter(function (oObject) {
								return !oList.getItems().some(function (oListItem) {
									return oListItem.getBindingContext().getProperty("Index") === oObject.Index;
								});
							});
							var sObjectNotInTheListId;

							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].Index;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].Index;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZET_GLHR_PERIODOS", {
								Index: sObjectNotInTheListId
							});
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Index"),
						title: oBindingContext.getProperty("Pernr")
					};
				}
			},
			...
			assertions: {
				...
				theListShouldBeFilteredOnUnitNumber: function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("Index", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber: function () {
					return this.theListShouldBeSortedAscendingOnField("Index");
				},

				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Pernr");
				},
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("ZET_GLHR_PERIODOS");
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<ZET_GLHR_PERIODOS> (" + iExpectedLength + ")"
								}),
								success: function () {
