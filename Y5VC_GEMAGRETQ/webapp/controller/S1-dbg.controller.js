sap.ui.define([
	...
	return Controller.extend("Workspace.zagrupador.controller.S1", {
		myFormatter: Formatter,

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf Workspace.zagrupador.view.S1
		 */
		...
		_processarFinalizar: function () {
			...
			oModelUtil.callFunction("/finalizarAgrupador", {
				...
				success: function (oData, response) { // callback function for success
					...
					MessageToast.show(oBundle.getText("PopFinalizarConfirm", [vAgrupadorOut]));
		...
		_PDF: function (that, vAgrupador) {
			...
			var string = that.getView().getModel("GE").sServiceUrl +
				"/ZET_VCMM_FILESet(fileName='" + oData.Agrupador + "',fileCategory='AGR',fileDescription='PE')/$value";
