sap.ui.define([
	...
	"Y5GL_DECLAR4/Y5GL_DECLAR4/test/integration/pages/Common",
			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Begda") === oObject.Begda;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Begda;
									} else {
										sCurrentId = aItemsNotInTheList[0].Begda;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZET_GLHR_APROVARSet", {
										Begda : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Begda"),
						title: oBindingContext.getProperty("Begda")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<ZET_GLHR_APROVARSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Begda");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ZET_GLHR_APROVARSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<ZET_GLHR_APROVARSet> (" + iExpectedLength + ")"}),
								success : function () {
