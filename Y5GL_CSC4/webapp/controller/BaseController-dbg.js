sap.ui.define([
	...
	return Controller.extend("Y5GL_CSC4.Y5GL_CSC4.controller.BaseController", {
		/**
		...
		Aprova: function () {
			...
			var dialog = new Dialog({
				...
				type: "Message",
				content: new Text({
					text: "¿Confirma la efectividad de la solicitud?"
				}),
					...
					press: function () {
						oModel.update(Key, oEntry, {
							success: function (oData, oResponse) {
								sap.m.MessageBox.success("Solicitud completada", {
		...
		Reprova: function () {
			...
			var dialog = "";
			dialog = new Dialog({
				title: "Razón de la desaprobación",
				type: "Message",
				content: [
					new Label({
						text: "Describe el motivo de la desaprobación.",
				...
				beginButton: new Button({
					...
					press: function () {
						if (oEntry.Motivo === undefined) {
							sap.m.MessageBox.error("Razón no llena");
							return;
						} else {
							dialog.close();
							oModel.update(Key, oEntry, {
								success: function (oData, oResponse) {
									sap.m.MessageBox.success("Solicitud rechazada", {
										actions: ["OK", sap.m.MessageBox.Action.CLOSE],
										onClose: function (sAction) {
											that.getView().getModel().refresh(true);
											that.getRouter().navTo("worklist");
