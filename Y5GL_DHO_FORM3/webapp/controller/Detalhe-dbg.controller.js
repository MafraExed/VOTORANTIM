sap.ui.define([
	...
	return BaseController.extend("Y5GL_DHO_FORM3.Y5GL_DHO_FORM3.controller.Detalhe", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf Y5GL_DHO_FORM3.Y5GL_DHO_FORM3.view.Detalhe
		 */
		onInit: function () {
		...
		onBeforeUploadStarts: function (oEvent) {
			...
			if (Infty !== "" && Subty !== "") {
				...
				sap.m.MessageBox.error("No se pudo cargar el archivo.");
			}
			//			_busyDialog.open();
		...
		onuploadComplete: function (oEvent) {
			...
			sap.m.MessageBox.success("Su formulario se ha adjuntado correctamente.");
		},
		onChange: function (oEvent) {
			var Bukrs = this.getView().byId("ComboBukrs").getSelectedKey();
			if (Bukrs === "") {
				sap.m.MessageBox.error("Informar a la empresa antes de cargar el archivo.");
				return;
			}

			var Area = this.getView().byId("ComboArea").getSelectedKey();
			if (Area === "") {
				sap.m.MessageBox.error("Informar al área de RH antes de cargar el archivo.");
				return;
			}
		...
		onDeleteSelectedItems: function (oEvent) {
			...
			oModel.update(Key, oEntry, {
				success: function (oData, oResponse) {
					sap.m.MessageBox.success("Archivo eliminado correctamente.", {
						actions: ["OK"],
