sap.ui.define([
	...
	"./nfItemModel",
	"./nfLabelModel"
], function (Object, JSONModel, BaseObject, NfHeaderModel, NfItemModel, NfLabelModel) {
	"use strict";
	const cService = "/ZET_VCMM_NFHEADERSet";
	const cNavHeadToItem = "NAVHEADITEM";
	const cNavHeadToLabel = "NAVHEADLABEL";
	...
	var nfHeaderList = BaseObject.extend("com.sap.build.standard.operadorLogistico.model.NFHeaderListModel", {
		...
		getNfLabelModelList: function (vChvNfe) {
			var oNfHeader = this.findByChvnfe(vChvNfe);
			return oNfHeader.getNfLabelModelList();
		},
		getNfLabelScanModelList: function (vChvNfe) {
			var oNfHeader = this.findByChvnfe(vChvNfe);
			return oNfHeader.getNfLabelScanModelList();
		},
		getLines: function () {
		...
		updateNfHeaderFromResult: function (oNfHeader, oResult) {
			...
			if (oResult.NAVHEADLABEL) {
				for (let oResultLabel of oResult.NAVHEADLABEL.results) {
					let oNfLabelModel = new NfLabelModel(oResultLabel.Chvnfe);
					oNfLabelModel.setNretq(oResultLabel.nretq);
					oNfLabelModel.setChvnfe(oResultLabel.chvnfe);
					oNfLabelModel.setItmnum(oResultLabel.itmnum);
					oNfLabelModel.setCatEtq(oResultLabel.categoriaEtq);
					oNfLabelModel.setStatus(oResultLabel.status);
					oNfLabelModel.setMenge(oResultLabel.menge);
					oNfLabelModel.setMeins(oResultLabel.meins);
					oNfLabelModel.setCheckBox(oResultLabel.checkbox);
					oNfHeader.addNfLabel(oNfLabelModel);
				}
			}
		},
		// Read Memory
		readNfHeaderByChvNfe: function (oView, cSuccess, vChvNfe) {
			...
			if (oNfHeader) {
				cSuccess(oView, oNfHeader.getModel(), oNfHeader.getNfItemModelList(), oNfHeader.getNfLabelModelList());
				return;
			} else {
		...
		readData: function (oView, cSuccess, aFilters, cError, that, vChvNfe) {
			...
			var vExpand = cNavHeadToLabel.concat(",").concat(cNavHeadToItem);

			//Faz Requisição ao backend
			oModel.read(cService, {
				filters: aFilters,
				urlParameters: {
					// $expand: cNavHeadToLabel,
					// 		 cNavHeadToItem
					$expand: vExpand
				},
		...
		readEntitiesByChvNFe: function (oView, cSuccess, cError, that, vChvNfe) {
			...
			this.readData(oView, cSuccess, aFilter, cError, that);
		},
