...
sap.ui.define([
  ...
  "vsa/y5gl_lp_portal/model/grouper",
  "vsa/y5gl_lp_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_lp_portal.view.Master", {
...
//      var l = this._oList.getItems().length;
//      for(var x=0;x<l;x++){
//        var z = i[x].getBindingContext().getObject().Z4cpeZterm; 
//        if(z != ""){
//          i[x].destroyMarkers();
//          i[x].removeMarker();
//          i[x].addMarker(new sap.m.ObjectMarker({
...
//      });
    ...
    onSearch: function (oEvent) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.Dmkey.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.Descricao.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
            ) {
            if(!bFound){
...
onMultiSelectPress: function (e) {
  ...
  if(this.getRouter().oHashChanger.getHash().indexOf("mLp") != -1){
    this._showDetail(this._oList.getItems()[0]);
  }
  } else {
  this._oList.setMode('MultiSelect');
  ...
  if(!Device.system.phone){;
    ...
    onSemanticFilterPress: function(){
         ...
         if (!this._oDialogApprove_S) {
           this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_lp_portal.view.fragment.advsearch", this);
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_lp_portal.view.fragment.filter", this);
...
//          }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      var sLblni,
        sUnidade = undefined;
      if(oItem){
        sLblni = oItem.getBindingContext().getProperty("Dmkey")
