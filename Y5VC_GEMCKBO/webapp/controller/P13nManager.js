sap.ui.define([
	...
	"use strict";
	return BaseObject.extend("workspace.zcockpit_bo_v3.controller.P13nManager", {
		...
		getColumns: function () {
			var aColumnsItems = [];
			if (!this._oColumnsPanel) {
				return aColumnsItems;
			}
			aColumnsItems = this._oColumnsPanel.getColumnsItems();
			return aColumnsItems;
		},
		_getDialog: function () {
		...
		_createP13nGroupPanel: function () {
			...
			this._oGroupPanel.setMaxGroups(1); //Máximo
		...
		_createP13nItemFactory: function (sId, oContext) {
			...
			var vText = oContext.getProperty("sColumnText");
			if (vText) {
				oItem.setProperty("text", vText);
			} else {
