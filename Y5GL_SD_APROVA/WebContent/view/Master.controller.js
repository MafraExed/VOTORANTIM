...
sap.ui.define([
  ...
  "vsa/y5gl_sd_portal/model/grouper",
  "vsa/y5gl_sd_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_sd_portal.view.Master", {
...
//          }
        ...
        if (this._oList.getItems().length > 0){
          ...
          if (this.sName === "sSd") {
            ...
            for (var i = 0; i < aItems.length; i++) {
              ...
              if (oObject.Ovkey === this.sOvkey && oObject.Unidade === this.sUnidade) {
    ...
    onSearch: function (oEvent) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.Vbeln.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.DescrKunnr.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
            ) {
            if(!bFound){
    ...
    onSemanticFilterPress: function(){
         if (!this._oDialogApprove_S) {
           this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_sd_portal.view.fragment.advsearch", this);
...
onMultiSelectPress: function (e) {
  ...
  if(this.getRouter().oHashChanger.getHash().indexOf("mSd") != -1){
    this._showDetail(this._oList.getItems()[0]);
  }
  } else {
  this._oList.setMode('MultiSelect');
  ...
  if(!Device.system.phone){;
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_sd_portal.view.fragment.filter", this);
    ...
    _onMasterMatched: function (oEvent) {
      ...
      this.sOvkey = (oParams && oParams.ovkey ? oParams.ovkey : "" );
...
//          }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      if(oItem){
        ...
        sOvkey   = oItem.getBindingContext().getProperty("Ovkey");
...
//        sUnidade = this.getRouter().oHashChanger.getHash().substring(15, 19);
    ...
    _showMulti: function (oItem, bInitial) {
      ...
      if(bInitial){
        ...
        this.getRouter().navTo("mSd", {
          ovkey   : oItem.getBindingContext().getProperty("Ovkey"),
