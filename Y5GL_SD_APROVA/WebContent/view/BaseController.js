...
sap.ui.define([ "sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_sd_portal/model/formatter" ], function(Controller, History, formatter) {
  "use strict";

  return Controller.extend("vsa.y5gl_sd_portal.view.BaseController", {
    ...
    setUser : function(sUserId) {
      ...
      if(sUserId == ""){
        this._oList.bindItems({
          ...
          parameters: {
             expand: "ToAprovadores,ToDadosAdicionais"
          }
        });
      }else{
        this._oList.bindItems({
        ...
        success : function(oData, response) {
          if(oData.results.length > 0){
            if(oData.results[0].EAdm && oData.results[0].EAdm != ""){
              ...
              if (!t._oDialogAdm) {
                t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_sd_portal.view.fragment.adm", t);
...
//        });
    ...
    setComentario : function(sPath, sOvkey, sUnidade) {
      var t = this;
      var sText = "";
      this.getOwnerComponent().getModel().read("/Comentarios", { // COMENT�RIOS
        async : true,
        filters : [
              new sap.ui.model.Filter("Ovkey", "EQ", sOvkey),
        ...
        success : function(oData, response) {
          ...
          t.getOwnerComponent().getModel("global").setProperty("/" + sOvkey, sText);
    ...
    setIconTab : function(oIconTabBar, sPath) {
      if(oIconTabBar){
        ...
        oIconTabBar.bindAggregation("items", sPathToAprova, function(index, context) {
          ...
          if(context.getObject().Status != "A" && context.getObject().Status != "C"){
            return new sap.m.IconTabFilter({
              ...
              text : { path: 'NameUsr' },
              content : new sap.m.Text({text: t.getOwnerComponent().getModel("global").getProperty("/" + context.getObject().Ovkey)})
            });
            ...
            var fil = new sap.ui.model.Filter({ filters: [ new sap.ui.model.Filter("IUnidade", "EQ", context.getObject().Unidade),
                                     new sap.ui.model.Filter("IDocumento", "EQ", context.getObject().Ovkey),
                                     new sap.ui.model.Filter("IProcesso", "EQ", "SD"),
          ...
          }else if(context.getObject().Status === "C"){
            return new sap.m.IconTabFilter({
              icon : { path: 'Status', formatter : formatter.iconTabBar } ,
              iconColor : { path: 'Status', formatter : formatter.iconColor }, 
              count : { path: 'Relcode' },
              tooltip: { path: 'NameUsr' }, 
              text : { path: 'NameUsr' },
              content : sap.ui.xmlfragment("vsa.y5gl_sd_portal.view.fragment.adddata", t)
            });
          }
    ...
    _readKPI: function(sPath){
      var sPathToAdicionais = sPath + "/ToDadosAdicionais";

      this.getOwnerComponent().getModel().read(sPathToAdicionais,{
        success: function(oData){
          this._buildKPI(oData);
        }.bind(this),
        error: function(oError){}
      });
    },

    _buildKPI: function(oData){
      // Base
      var oDataKPI = {
        Traco: [],
        Bomba: [],
      ...
      oData.results.forEach( function(oEntry){
        // Tipo de Entrada 
        switch (oEntry.ZIdentificador){
          case "TRACO":
            oDataKPI.Traco.push({
              Filial: oEntry.ZWerksTexto,
              Traco: oEntry.ZMatnrTexto,
              PrecoNegociado: oEntry.ZPrecoNegoc,
              Volume: oEntry.ZVolume,
              MargemLivre: oEntry.ZMargemLivre,
              Desconto: oEntry.ZDesconto,
              PrecoRegra: oEntry.ZPrecoRegra
            });
            break;
          case "BOMBA":
            oDataKPI.Bomba.push({
              Filial: oEntry.ZWerksTexto,
              Bomba: oEntry.ZMatnrTexto,
              PrecoNegociado: oEntry.ZPrecoNegoc,
              Volume: oEntry.ZVolume,
              MargemLivre: oEntry.ZMargemLivre,
              Desconto: oEntry.ZDesconto,
            ...
            break;
          case "DADIC_01":
            oDataKPI.Adicionais.push({
              Adicional: oEntry.ZSubIdentificador === "HORA_EXTRA"    ? this.getResourceBundle().getText("HoraExtra") : 
                     ( oEntry.ZSubIdentificador === "ADIC_FALTA"    ||
                     oEntry.ZSubIdentificador === "ADIC_FALTANTE" ) ? this.getResourceBundle().getText("AdicionalM3Restante") :
                     ( oEntry.ZSubIdentificador === "TX_MIN_BOM"    ) ? this.getResourceBundle().getText("TaxaMinima") :
                     ( oEntry.ZSubIdentificador === "COND_PAGTO"    ) ? this.getResourceBundle().getText("CondicaoPagamento")   :
                     this.getResourceBundle().getText("DataLimite"),
                Regra: oEntry.ZDadosAdic01Regra,
                Proposta: oEntry.ZDadosAdic01Prop              
            });
            break;
          case "DADIC_02":
            oDataKPI.Acrescimo.push({
              Margem: oEntry.ZAcrescimoTexto,
              Percentual: oEntry.ZAcrescimoValor
            });
            break;
          case "MARGEM":
            oDataKPI.Margem.push({
              Margem: oEntry.ZMargemTexto,
              Valor: oEntry.ZMargemValor
            });
            break;
          case "RECOMENDAC" || "RECOMENDACAO":
            if ( oEntry.ZSubIdentificador === "CONTRATO" ){
                oDataKPI.Recomendacao.push({
                  Recomendacao: oEntry.ZRecomendTexto
              });
            }else if (oEntry.ZSubIdentificador === "BOMBA" ){
              oDataKPI.RecomendacaoBomba.push({
                Recomendacao: oEntry.ZRecomendTexto
              });
            }
            break;
          case "FAMILIA":
            oDataKPI.Familia.push({
              Familia: oEntry.ZFamiliaTexto,
              Percentual: oEntry.ZFamiliaValor
            });
            break;
          default:
            break;
      ...
      this.getView().getModel("KPIView").setData(oDataKPI);
      this.getView().getModel("KPIView").refresh(true);
    },
