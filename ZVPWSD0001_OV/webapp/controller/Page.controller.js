sap.ui.define([
	...
	"use strict";
	var _timeout;
	return Controller.extend("zvpwsd0001_ov.controller.Page", {
		...
		createPopover: function() {
			if (!this._oQuickView) {
				this._oQuickView = sap.ui.xmlfragment("zvpwsd0001_ov.fragment.Grupo", this);
		...
		onRejeitar: function(oEvent) {
			...
			if (!this._dialog) {
				var vBusyDialog = sap.ui.xmlfragment("zvpwsd0001_ov.fragment.BusyDialogRej", this);
				this.getView().addDependent(vBusyDialog);
			}
			var oModelP = this.getView("default").getModel();
			var IdDocumento = oEvent.getSource().getBindingContext().getProperty("Processo");
		...
		_onRouteMatched: function(oEvent) {
			...
			oView = this.getView();
			oView.bindElement({
				path: "/OrdemVendas(Processo='" + oArgs.Processo + "',Documento='" + oArgs.Documento + "')",
				events: {
		...
		handleResponsivePopoverPress: function(oEvent) {
			//	if (!this._oPopover) {
			var oView = this.getView();
			this._oPopover = sap.ui.xmlfragment(oView.getId(), "zvpwsd0001_ov.fragment.Grupo", this);
		...
		onHistory: function(oEvent) {
			...
			var oView = this.getView();
			this._oPopover = sap.ui.xmlfragment(oView.getId(), "zvpwsd0001_ov.fragment.History", this);
			this.getView().addDependent(this._oPopover);
			this._oPopover.openBy(oEvent.getSource());
		...
		onMessagePopOver: function(oArgs) {
			...
			var that = this;
			var oModel = this.getView().getModel();
			var sPath = ("/OrdemVendas(Processo='" + oArgs.Processo + "',Documento='" + oArgs.Documento + "')" + "/ToLog");
			oModel.read(sPath, {
				success: function(oData, oResponse) {
