sap.ui.define([
	...
	return BaseController.extend("Y5GL_EC_CAD3.Y5GL_EC_CAD3.controller.Documentos_Detail", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf Y5GL_EC_CAD3.Y5GL_EC_CAD3.view.Documentos_Detail
		 */
		onInit: function () {
		...
		onVoltar: function () {
			this.getRouter().navTo("DOCUMENTOS");
		...
		_onObjectMatched: function (oEvent) {
			...
			var Stext = oEvent.getParameter("arguments").Stext;
			...
			this.getView().byId("idTitleDependentes").setText(Stext);
		...
		_onBindingChange: function () {
			...
			this.getView().byId("_0001").setVisible(false);
			this.getView().byId("_0002").setVisible(false);
			this.getView().byId("_0003").setVisible(false);
			this.getView().byId("_0004").setVisible(false);
			this.getView().byId("_0005").setVisible(false);
			this.getView().byId("_0006").setVisible(false);
			this.getView().byId("_0007").setVisible(false);
			this.getView().byId("_0008").setVisible(false);
			this.getView().byId("_0009").setVisible(false);
			this.getView().byId("_0010").setVisible(false);
			this.getView().byId("_0011").setVisible(false);
			this.getView().byId("_0012").setVisible(false);
			this.getView().byId("_0014").setVisible(false);
			this.getView().byId("_0015").setVisible(false);
			this.getView().byId("_0001").setVisible(false);
		...
		onSave: function () {
			...
			case "_0002":
				...
				if(!oEntry.IdentNr){
					sap.m.MessageBox.error("campo 'Documento Identidade' é obrigatório");
					return;
				}
				...
				if(!oEntry.EsEmis){
					sap.m.MessageBox.error("campo 'UF de Expedição' é obrigatório");
					return;
				}
				break;
			case "_0003":
				...
				if(!oEntry.CtpsNr){
					sap.m.MessageBox.error("campo 'Número da CTPS' é obrigatório");
					return;
				}
				oEntry.CtpsSerie = this.getView().byId("idCtpsSerie").getValue();
				if(!oEntry.CtpsSerie){
					sap.m.MessageBox.error("campo 'Série da CTPS' é obrigatório");
					return;
				}
			...
			case "_0005":
				...
				if(!oEntry.ElecNr){
					sap.m.MessageBox.error("campo 'Nr. Título Eleitor' é obrigatório");
					return;
				}
				oEntry.ElecZone = this.getView().byId("idElecZone").getValue();
				if(!oEntry.ElecZone){
					sap.m.MessageBox.error("campo 'Zona Tít. Eleitor' é obrigatório");
					return;
				}
			...
			case "_0006":
				...
				if(!oEntry.PisNr){
					sap.m.MessageBox.error("campo 'PIS/PASEP' é obrigatório");
					return;
				}
			...
			case "_0007":
				...
				if(!oEntry.MilNr){
					sap.m.MessageBox.error("campo 'Nr. Cert. Militar' é obrigatório");
					return;
				}
				oEntry.MilType = this.getView().byId("idMilType").getValue();
				if(!oEntry.MilType){
					sap.m.MessageBox.error("campo 'Espécie Cert. Mil' é obrigatório");
					return;
				}
				oEntry.MilCat = this.getView().byId("idMilCat").getValue();
				if(!oEntry.MilCat){
					sap.m.MessageBox.error("campo 'Categoria Cert. Mil.' é obrigatório");
					return;
				}
			...
			case "_0008":
				...
				if(!oEntry.IdforNr){
					sap.m.MessageBox.error("campo 'Nr. ID Estrangeiro' é obrigatório");
					return;
				}
			...
			case "_0009":
				...
				if(!oEntry.VisaNr){
					sap.m.MessageBox.error("campo 'Nr. Série Visto' é obrigatório");
					return;
				}
			...
			case "_0010":
				...
				if(!oEntry.DriveNr){
					sap.m.MessageBox.error("campo 'Nr. Cart. Habilit.' é obrigatório");
					return;
				}
				oEntry.Cnhorg = this.getView().byId("idCnhorg").getValue();
				if(!oEntry.Cnhorg){
					sap.m.MessageBox.error("campo 'Órgão emissor CNH' é obrigatório");
					return;
				}
				oEntry.DriveCat = this.getView().byId("idDriveCat").getSelectedKey();
				if(!oEntry.DriveCat){
					sap.m.MessageBox.error("campo 'Categoria Habilit.' é obrigatório");
					return;
				}
				oEntry.DtEmis = this.getView().byId("idDtEmis_10").getValue();
				if(!oEntry.DtEmis){
					sap.m.MessageBox.error("campo 'Data Emissão' é obrigatório");
					return;
				}
				...
				if(!oEntry.Zvalidade){
					sap.m.MessageBox.error("campo 'Data validade' é obrigatório");
					return;
				}
				oEntry.Zphab = this.getView().byId("idZphab").getValue();
				if(!oEntry.Zphab){
					sap.m.MessageBox.error("campo 'Data prim. habilit' é obrigatório");
					return;
				}
				break;
			case "_0011":
				...
				if(!oEntry.VisaType){
					sap.m.MessageBox.error("campo 'Tipo de Visto' é obrigatório");
					return;
				}
				oEntry.DtArrv = this.getView().byId("idDtArrv_11").getValue();
				if(!oEntry.DtArrv){
					sap.m.MessageBox.error("campo 'Dt. chegada Brasil' é obrigatório");
					return;
				}
				oEntry.PasspNr = this.getView().byId("idPasspNr_11").getValue();
				if(!oEntry.PasspNr){
					sap.m.MessageBox.error("campo 'Número do Passaporte' é obrigatório");
					return;
				}
			...
			case "_0015":
				...
				if(!oEntry.Nhcnr){
					sap.m.MessageBox.error("campo 'Cart. Nacional Saúde' é obrigatório");
					return;
				}
				break;
			}

			var key = "/ZET_GLHR_MEUS_DOCUMENTOSSet(IPernr='0',IInfo='" + id2 + "',ITipo='G',Chamado='0')";
