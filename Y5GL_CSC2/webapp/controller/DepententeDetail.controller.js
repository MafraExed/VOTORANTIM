sap.ui.define([
	...
	"sap/ui/Device",
	"Y5GL_CSC2/Y5GL_CSC2/model/formatter",
	"sap/m/Button",
	"sap/m/Dialog",
	"sap/m/Text",
	"sap/m/Label",
...
], function (BaseController, Device, formatter, Button, Dialog, Text, Label, RadioButton, Filter, JSONModel) {
	"use strict";
	...
	return BaseController.extend("Y5GL_CSC2.Y5GL_CSC2.controller.DepententeDetail", {
		formatter: formatter,
		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf Y5GL_CSC2.Y5GL_CSC2.view.DepententeDetail
		 */
		onInit: function () {
			this.getRouter().getRoute("DependenteDetail").attachPatternMatched(this._onObjectMatched, this);

			// Image loading
			var sRootPath = jQuery.sap.getModulePath("Y5GL_CSC2.Y5GL_CSC2");
		...
		onSave: function () {
			...
			var Favor = this.getView().byId("IdFavor").getValue();
			var Cpf = this.getView().byId("idCpf").getValue();
			var Zplanosaude = this.getView().byId("IdPlano").getSelectedKey();
			var Fgbld = this.getView().byId("IdPaisNasc_Edit").getSelectedKey();
			var Ufbot = this.getView().byId("IdEstNasc_Edit").getSelectedKey();
			var Dtcvc = this.getView().byId("DtVencimentoVacina").getValue();
			var Noliv = this.getView().byId("nLivro").getValue();
			var Nhcnr = this.getView().byId("CNS").getValue();
			var Irflg = this.getView().byId("IdIR").getSelectedKey();
			var Carto = this.getView().byId("idFormCartorio").getValue();
			var Noreg = this.getView().byId("idFormRegistro").getValue();
			var Dtent = this.getView().byId("idFormDtEntrega").getValue();
			var Mothe = this.getView().byId("IdFormNomeMaeDep").getValue();
			var Lbcnr = this.getView().byId("IdFormNDeclarcVivo").getValue();
			var Fgbot = this.getView().byId("IdLocNascimento_Edit").getValue();
			if (!Fgbot) {
				this.getView().byId("IdLocNascimento_Edit").setValueState("Error");
				sap.m.MessageBox.error("Informe o local de nascimento.");
				return;
			} else {
				this.getView().byId("IdLocNascimento_Edit").setValueState("Success");
			}

			var Salfa = this.getView().byId("IdFormSalFamilia").getSelectedKey();
			if (Salfa === "N" || Salfa === "") {
				Salfa = "";
			} else {
				Salfa = "X";
			}
			var Fuman = this.getView().byId("idFumante").getSelectedKey();
			if (Fuman === "false" || Fuman === "N") {
				Fuman = false;
			} else {
				Fuman = true;
			}

			var Estud = this.getView().byId("idEstud").getSelectedKey();
			if (Estud === "false" || Estud === "N") {
				Estud = false;
			} else {
				Estud = true;
			}

			var Escol = this.getView().byId("IdFormGInstrucao").getSelectedKey();

			if (Escol === "") {
				this.getView().byId("IdFormGInstrucao").setValueState("Error");
				sap.m.MessageBox.error("Selecione o Grau de instrução.");
				return;
			} else {
				this.getView().byId("IdFormGInstrucao").setValueState("Success");
			}

			var Moden = this.getView().byId("IdFormModalidade").getSelectedKey();

			var Saled = this.getView().byId("IdFormSalEducacao").getSelectedKey();
			if (Saled === "N" || Saled === "") {
				Saled = "";
			} else {
				Saled = "X";
			...
			oEntry.Operation = this.getView().byId("idOperacao").getValue();

			if (oEntry.Operation !== "Exclusão") {
				oEntry.Fgbot = Fgbot;
				...
				oEntry.Fgbld = Fgbld;
				if (oEntry.Fgbld === "") {
					sap.m.MessageBox.error("Para aprovação deverá ser informado país de nascimento.");
					this.getView().byId("IdPaisNasc_Edit").setValueState("Error");
					return;
				} else {
					this.getView().byId("IdPaisNasc_Edit").setValueState("None");
				}
				oEntry.Ufbot = Ufbot;
				if (oEntry.Ufbot === "") {

					sap.m.MessageBox.error("Para aprovação deverá ser informado Estado de nascimento.");
					this.getView().byId("IdEstNasc_Edit").setValueState("Error");
					return;

				} else {
					this.getView().byId("IdEstNasc_Edit").setValueState("None");
				}
				oEntry.Dtcvc = Dtcvc;
				oEntry.Noliv = Noliv;
				oEntry.Nhcnr = Nhcnr;
				...
				if (oEntry.Mothe === "") {
					...
					sap.m.MessageBox.error("Para aprovação deverá ser informado  o nome da mãe do dependente.");
					this.getView().byId("IdFormNomeMaeDep").setValueState("Error");
					return;

				} else {
					this.getView().byId("IdFormNomeMaeDep").setValueState("None");
				}
				oEntry.Lbcnr = Lbcnr;
				oEntry.Salfa = Salfa;
				oEntry.Fuman = Fuman;
		...
		handleValueHelp: function (oEvent) {
			...
			if (!this._valueHelpDialog1) {
				this._valueHelpDialog1 = sap.ui.xmlfragment("Y5GL_CSC2.Y5GL_CSC2.view.LocNascimento", this);
		...
		onConfirmDocumento: function (evt) {
			...
			if (oSelectedItem) {
				...
				this.getView().byId("IdLocNascimento_Edit").setValue(sValue);
