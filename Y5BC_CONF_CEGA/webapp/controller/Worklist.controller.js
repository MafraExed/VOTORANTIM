...
sap.ui.define([
	...
	"Y5BC_CONF_CEGA/Y5BC_CONF_CEGA/model/formatter",
	"sap/ui/model/Filter",
	...
	"sap/m/MessageBox",
	"sap/m/Text"
], function (BaseController, JSONModel, formatter, Filter, FilterOperator, Dialog, Button, MessageToast, MessageBox, Text) {
	"use strict";

	return BaseController.extend("Y5BC_CONF_CEGA.Y5BC_CONF_CEGA.controller.Worklist", {
		...
		_reimprimirRecebimento: function (oModel, oImpressaoModel) {
			...
			// var serviceUrl = "/sap/opu/odata/sap/ZGWCBEWM_CONF_CEGA_SRV/ZET_CBEWM_FORNECIMENTOCBSet/$count?$filter=AvisoEntrega eq '" +
			// 	aviso + "'";
		...
		_impressaoManual: function (oModel, oImpressaoModel) {
			...
			for (var i = 0; i < items.length; i++) {
				...
				var dep = this.getView().byId("ins").getItems()[i].getCells()[3].getValue();

				if ((!matnr) || (!qtd) || (!dep)) {
					MessageToast.show("Campos Material, Quantidade e Deposito são obrigatórios");
					return;
				}
				var serviceUrl = "/sap/opu/odata/sap/ZGWCBEWM_CONF_CEGA_SRV/ZET_CBEWM_SH_MATNRSet/$count?$filter=Matnr eq '" +
					matnr + "'";
			...
			for (i = 0; i < items.length; i++) {
				...
				var lote = this.getView().byId("ins").getItems()[i].getCells()[4].getValue();
				var pep = this.getView().byId("ins").getItems()[i].getCells()[5].getValue();
		...
		onContCegPress: function (oEvent) {
			...
			if (!this._oDialog) {
				this._oDialog = sap.ui.xmlfragment("Y5BC_CONF_CEGA.Y5BC_CONF_CEGA.fragment.NovosValoresContagemCega", this);
		...
		onDialogSavePress: function (oEvent) {
			...
			if (Erro === 0) {
				...
				sap.m.MessageBox.error("Informar pedido e item no item da Nota Fiscal");
			}
		...
		onFornecimentoListItemPress: function (oEvent) {
			...
			if ((oContext.getProperty("ErroContagem") && oContext.getProperty("ContagemRealizada")) ||
				...
				this.getRouter().navTo("fornecimento", {
				    ...
				    erroContagem: oContext.getProperty("ErroContagem")
				});
		...
		AfterUpdate: function (oEvent) {
			if (oEvent.getSource().getModel().getMessagesByEntity("/ZET_CBEWM_CONF_EFETUADASet")[0].message) {	
				var Menssagem = oEvent.getSource().getModel().getMessagesByEntity("/ZET_CBEWM_CONF_EFETUADASet")[0].message;
			}
			if (Menssagem) {
				sap.m.MessageBox.error(Menssagem);
				oEvent.getSource().getModel().getMessagesByEntity("/ZET_CBEWM_CONF_EFETUADASet")[0].message = "";
			}
		},

		AfterUpdate2: function (oEvent) {

			if (oEvent.getSource().getModel().getMessagesByEntity("/ZET_CBEWM_FORNECIMENTOCBSet")[0].message) {
				var Menssagem = oEvent.getSource().getModel().getMessagesByEntity("/ZET_CBEWM_FORNECIMENTOCBSet")[0].message;
			}

			if (Menssagem) {
				sap.m.MessageBox.error(Menssagem);
				oEvent.getSource().getModel().getMessagesByEntity("/ZET_CBEWM_FORNECIMENTOCBSet")[0].message = "";
			}
		}
