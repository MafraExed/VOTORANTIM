...
sap.ui.define([
		...
		return Controller.extend("nasa.ui5.monitorEmbarques.controller.BaseController", {
			/**
			...
			onMessageDisplay: function(pType, pId){
			    ...
			    case "A":
			        ...
			        break;
			    case "W":
		        	MessageBox.warning(sMsg, { styleClass: this.getOwnerComponent().getContentDensityClass() } );
			        break;			        
				};	
			},	          
			
			/**
			 * Popup com mensagem de confirma��o
			 * @public
			 */
			...
			/* =========================================================== */
  			shlp_create_filter: function(oEvent){
                ...
                if(oEvent.getId() == "submit"){
                	//"Enter" no campo Input
                	if(oEvent.getParameters().id.indexOf("Ptdst") !== -1){
                		var sProperty = "Ptdst";
                		var sOperator = sap.ui.model.FilterOperator.Contains;
                	}else if(oEvent.getParameters().id.indexOf("Route") !== -1){
                		var sProperty = "Route";
                		var sOperator = sap.ui.model.FilterOperator.EQ;
                	}else{
                		var sProperty = "PiName";
                		var sOperator = sap.ui.model.FilterOperator.Contains;
                	}

                }else{
                	//Pesquisa da Ajuda de Pesquisa
                	var sProperty = oEvent.getParameters().itemsBinding.sPath == '/ZET_FBSD_SearchPortSet' ? "Ptdst" : "PiName";
                	var sOperator = sap.ui.model.FilterOperator.Contains;
                }

                if (sValue) {
                  var oFilter = null;
                  oFilter = new sap.ui.model.Filter([ new sap.ui.model.Filter(sProperty, sOperator, sValue)], false);
			...
			onHandleF4Global: function(oDialog, oView, oFragment){
                ...
                if (! oDialog) {
                	oDialog = sap.ui.xmlfragment("nasa.ui5.monitorEmbarques.view.fragments."+oFragment, this);
                }
