sap.ui.define([
	...
	return BaseController.extend("Y5GL_CADASTRO.Y5GL_CADASTRO.controller.BENEFICIOS_DETAIL", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf Y5GL_CADASTRO.Y5GL_CADASTRO.view.BENEFICIOS_DETAIL
		 */
		onInit: function () {
		...
		_onObjectMatched: function (oEvent) {
			...
			var Pernr = "0";
			this.getView().byId("idTitleDependentes").setText(Beneficio);
		...
		onchangeIdMargemEmp: function (oEvent) {
			...
			if (number === true) {
				...
				sap.m.MessageBox.error("O valor informado deverÃ¡ ser um nÃºmero.", {
		...
		onCancelaAprovacao: function () {
			...
			dialog = new Dialog({
				...
				type: "Message",
				content: new Text({
					text: "Confirma a exclusÃ£o da solicitaÃ§Ã£o do benefÃ­cio?"
				}),
					...
					press: function () {
						oModel.update(Key, oEntry, {
							success: function (oData, oResponse) {
								...
								sap.m.MessageBox.success("SolicitaÃ§Ã£o de benefÃ­cio excluida.", {
									actions: ["OK", sap.m.MessageBox.Action.CLOSE],
									onClose: function (sAction) {
			...
			dialog = new Dialog({
				...
				beginButton: new Button({
					...
					press: function () {
						oModel.update(Key, oEntry, {
							success: function (oData, oResponse) {
								sap.m.MessageBox.success("SolicitaÃ§Ã£o excluida.", {
									actions: ["OK", sap.m.MessageBox.Action.CLOSE],
									onClose: function (sAction) {
										that.getRouter().navTo("BENEFICIOS");
									}
			...
			dialog = new Dialog({
				...
				beginButton: new Button({
					...
					press: function () {
						oModel.update(Key, oEntry, {
							success: function (oData, oResponse) {
								sap.m.MessageBox.success("BenefÃ­cio salvo com sucesso.", {
									actions: ["OK", sap.m.MessageBox.Action.CLOSE],
									onClose: function (sAction) {
										that.getRouter().navTo("BENEFICIOS");
									}
		...
		onSave: function () {
			...
			case "VALE_TRANSPORTE":
				...
				if (oEntry.Opcoes === "") {
					...
					sap.m.MessageBox.error("Informe a opÃ§Ã£o");
					return;
				} else {
			...
			case "AUXILIO_CRECHE":
				...
				break;
			case "COOPERATIVA":
				oEntry.Rubrica = this.getView().byId("IdRubrica_COOPERATIVA").getSelectedKey();
			...
			case "ALIMENTACAO":
				...
				if (oEntry.Opcoes === "") {
					...
					sap.m.MessageBox.error("Informe as opÃ§Ãµes");
					return;
				} else {
