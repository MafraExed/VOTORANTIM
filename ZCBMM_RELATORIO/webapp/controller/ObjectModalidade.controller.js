...
sap.ui.define([
	...
	return BaseController.extend("ZCBMM_RELATORIO.ZCBMM_RELATORIO.controller.ObjectModalidade", {
		...
		onInit: function() {
			...
			this.getRouter().getRoute("objectModalidade").attachPatternMatched(this._onObjectMatched, this);
			this.getOwnerComponent().getModel().metadataLoaded().then(function() {
				// Restore original busy indicator delay for the object view
				oViewModel.setProperty("/delay", iOriginalBusyDelay);
			});
			this.oVizFrame = this.byId("idoVizFrameModalidade");
			//Popover
			var popoverProps = {};
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getModel().metadataLoaded().then(function() {
				this._bindView("/ZET_CBMM_CF_MODALIDADESet", filters);
			}.bind(this));
		},
		...
		_bindView: function(sObjectPath, filters) {
			...
			this.getView().byId("idoVizFrameModalidade").getDataset().getBinding("data").filter(filters).attachDataReceived(function(evt) {
