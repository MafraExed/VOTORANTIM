...
sap.ui.define([
	...
	return BaseController.extend("ZCBMM_RELATORIO.ZCBMM_RELATORIO.controller.ObjectSavings", {
		...
		onInit: function() {
			...
			this.getRouter().getRoute("objectSavings").attachPatternMatched(this._onObjectMatched, this);
			this.getOwnerComponent().getModel().metadataLoaded().then(function() {
				// Restore original busy indicator delay for the object view
				oViewModel.setProperty("/delay", iOriginalBusyDelay);
			});
			this.oVizFrame = this.byId("idoVizFrameSavings");
			//Popover
			var popoverProps = {};
		...
		onChangeChart: function(evt) {
			...
			switch (key) {
				case 'Scatter':
					this.oVizFrame.setVizType('timeseries_scatter');

					break;
				case 'Line':
					this.oVizFrame.setVizType('timeseries_line');

					break;
				case 'Column':
					this.oVizFrame.setVizType("timeseries_column");
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getModel().metadataLoaded().then(function() {
				this._bindView("/ZET_CBMM_CF_SAVINGSSet", filters);
			}.bind(this));
		},
		...
		_bindView: function(sObjectPath, filters) {
			...
			this.getView().byId("idoVizFrameSavings").getDataset().getBinding("data").filter(filters).attachDataReceived(function(evt) {
