/*global history */
sap.ui.define([
  "vsa/y5gl_sr_portal/view/BaseController",
  "sap/ui/model/json/JSONModel",
  "sap/ui/model/Filter",
  "sap/ui/model/FilterOperator",
  "sap/m/GroupHeaderListItem",
  "sap/ui/Device",
  "vsa/y5gl_sr_portal/model/formatter",
  "vsa/y5gl_sr_portal/model/grouper",
  "vsa/y5gl_sr_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_sr_portal.view.Master", {
...
//          }
    ...
    onUserPress: function(oEvent){
      ...
      var sTitle = oObject.Lblni;

      sTitle = this.getResourceBundle().getText("EmailTtl", sTitle);
      sap.m.URLHelper.triggerEmail(oObject.EMailErnan, sTitle);
    },
        ...
        if (this._oList.getItems().length > 0){
          ...
          if (this.sName === "sSr") {
            ...
            for (var i = 0; i < aItems.length; i++) {
              ...
              if (oObject.Lblni === this.sLblni && oObject.Unidade === this.sUnidade) {
    ...
    onSearch: function (oEvent) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.DescrWerks.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.Lblni.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
            ) {
            if(!bFound){
    ...
    onSemanticFilterPress: function(){
         if (!this._oDialogApprove_S) {
           this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_sr_portal.view.fragment.advsearch", this);
...
onMultiSelectPress: function (e) {
  ...
  if(this.getRouter().oHashChanger.getHash().indexOf("mSr") != -1){
    this._showDetail(this._oList.getItems()[0]);
  }
  } else {
  this._oList.setMode('MultiSelect');
  ...
  if(!Device.system.phone){;
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_sr_portal.view.fragment.filter", this);
    ...
    _onMasterMatched: function (oEvent) {
      ...
      this.sLblni = (oParams && oParams.lblni ? oParams.lblni : "" );
...
//          }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      var sLblni,
        sUnidade = undefined;
      if(oItem){
        sLblni = oItem.getBindingContext().getProperty("Lblni")
        sUnidade = oItem.getBindingContext().getProperty("Unidade")
      }else{
        sLblni = this.getRouter().oHashChanger.getHash().substring(4, 14);
        sUnidade = this.getRouter().oHashChanger.getHash().substring(15, 19);
      }
      this.getRouter().navTo("sSr", {
        lblni: sLblni,
        unidade: sUnidade
      }, bReplace);
    },

    _showMulti: function (oItem) {
      // var oData = [];
      // for (var i = 0; i <this._oList.getSelectedItems().length; i++) {
      //  oData.push(this._oList.getSelectedItems()[i].getBindingContext().getObject());
      // }
      // this.getOwnerComponent().getModel('multiModel').setData(oData);

      var bReplace = !Device.system.phone;
      this.getRouter().navTo("mSr", {
        lblni: oItem.getBindingContext().getProperty("Lblni"),
