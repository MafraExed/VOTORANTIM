...
sap.ui.define([
  ...
  return BaseController.extend("vsa.y5gl_cr_portal.view.Detail", {
    ...
    onInit: function () {
      ...
      this.getRouter().getRoute("sCr").attachPatternMatched(this._onObjectMatched, this);
    ...
    _onObjectMatched: function (oEvent) {
        ...
        this._sEbeln = oEvent.getParameter("arguments").numproposta;
    ...
    _bindView: function (sObjectPath) {
      ...
      oModel.read(sObjectPath + '/ToItems', {
        async : true,
    ...
    onApprove: function (e) {
      if (!this._oDialogApprove) {
        this._oDialogApprove = sap.ui.xmlfragment("vsa.y5gl_cr_portal.view.fragment.approve", this);
    ...
    onApprove_W: function (e) {
      if (!this._oDialogApprove_W) {
        this._oDialogApprove_W = sap.ui.xmlfragment("vsa.y5gl_cr_portal.view.fragment.approve_w", this);
    ...
    onReject: function (e) {
      ...
      aFilters.push(new Filter("Processo", FilterOperator.EQ, "CR"));
    ...
    _openRejectionPopup: function(){
      if (!this._oDialogReject) {
        this._oDialogReject = sap.ui.xmlfragment(this.getView().getId(),"vsa.y5gl_cr_portal.view.fragment.reject", this);
      ...
      if(oformReject){
        ...
        var oRbgReject = new sap.m.RadioButtonGroup({
          buttons: [ new sap.m.RadioButton({ text : oResource.getText("REJECTRB-1") }).addCustomData(new sap.ui.core.CustomData({key : "I"})),
                 new sap.m.RadioButton({ text : oResource.getText("REJECTRB-2") }).addCustomData(new sap.ui.core.CustomData({key : "N"}))
          ]
        });
    ...
    onConfirmReject : function(e){
      ...
      this.getOwnerComponent().getModel('global').setProperty("/rejEvent", sRejCode);
  ...
  onQuestion : function(e){
    ...
    if(sContext && sContext.EMail === ""){ return  sap.m.MessageToast.show(this.getResourceBundle().getText("EMail")); }
      if (!this._oDialogQuestion) {
        this._oDialogQuestion = sap.ui.xmlfragment("vsa.y5gl_cr_portal.view.fragment.question", this);
  ...
  onInterruption : function(e){
    ...
    if (!this._oDialogInterruption) {
      this._oDialogInterruption = sap.ui.xmlfragment("vsa.y5gl_cr_portal.view.fragment.interrupt", this);
