/*global history */
sap.ui.define([
  "vsa/y5gl_cr_portal/view/BaseController",
  "sap/ui/model/json/JSONModel",
  "sap/ui/model/Filter",
  "sap/ui/model/FilterOperator",
  "sap/m/GroupHeaderListItem",
  "sap/ui/Device",
  "vsa/y5gl_cr_portal/model/formatter",
  "vsa/y5gl_cr_portal/model/grouper",
  "vsa/y5gl_cr_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_cr_portal.view.Master", {
...
//          }
    ...
    onMultipleAction: function(){
      ...
      this.getRouter().navTo("mCr", {
        numproposta: "All",
        unidade: "All"
      }, bReplace);
    },
    
    _retrieveDocumentItems: function(aItems){
    	var sGroup = jQuery.sap.uid();
    	this.getModel().setDeferredGroups([sGroup]);
    	
    	aItems.forEach(function(oItem){
            var sObjectPathHeader = "/" + this.getModel().createKey("Propostas", {
                Unidade: oItem.Unidade,
                NumProposta: oItem.NumProposta
            });
            
            sObjectPathHeader += "/ToItems";
			this.getModel().read(sObjectPathHeader, {
				groupId: sGroup
			});            
    	}.bind(this));
		...
		return new Promise(function(fnResolve, fnReject) {
			this.getModel().submitChanges({
				groupId: sGroup,
				success: function(oDocumentItems) {
					fnResolve(oDocumentItems);
				}.bind(this),
    ...
    onUserPress: function(oEvent){
      ...
      var sTitle = oObject.NumProposta;
        ...
        if (this._oList.getItems().length > 0){
          ...
          if (this.sName === "sCr") {
            ...
            for (var i = 0; i < aItems.length; i++) {
              ...
              if (oObject.NumProposta === this.sNumProposta && oObject.Unidade === this.sUnidade) {
    ...
    onSearch: function (oEvent) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.NumProposta.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.Name1.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 
            ) {
            if(!bFound){
  ...
  onSemanticFilterPress: function(){
       if (!this._oDialogApprove_S) {
         this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_cr_portal.view.fragment.advsearch", this);
       ...
       // Valida se h� algum registro marcado para sa�da como filtro 
    ...
    onConfirmAdvSearch: function(){
      ...
      for ( var i = 0; i < aItems.length ; i++){
        ...
        if (oItemData.ToItems && oItemData.ToItems.__list && oItemData.ToItems.__list.length > 0 ){
          var sPath = "/" + oItemData.ToItems.__list[0];
    ...
    onMultiSelectPress: function (e) {
      ...
      if(this.getRouter().oHashChanger.getHash().indexOf("mCr") != -1){
        this._showDetail(this._oList.getItems()[0]);
      }
      } else {
      this._oList.setMode('MultiSelect');
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_cr_portal.view.fragment.filter", this);
    ...
    onSelectionChange: function (oEvent) { 
      ...
      if (this._oList.getMode() === "MultiSelect") {
        ...
        if( oArray.length === 0){
      	  ...
      	  this._retrieveDocumentItems(oArray).then(function(oBatchItems){      		        		 
    		  var aInnerItems = [];
    		  if (oBatchItems.__batchResponses){
    			  oBatchItems.__batchResponses.forEach(function(oBatchResponse){
    				  if (oBatchResponse.data.results && oBatchResponse.data.results.length > 0){
    					  aInnerItems.push(oBatchResponse.data.results["0"]);  
    				  }    				  
    			  }.bind(this));
    		  }
        	  this._oMultiModel.setData({ results : aInnerItems });
        	  this._showMulti(oSource);    		  
    	  }.bind(this));        	
        }
      } else {
    ...
    _onMasterMatched: function (oEvent) {
      ...
      this.sNumProposta = (oParams && oParams.numproposta ? oParams.numproposta : "" );
...
//          }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      var sEbeln,
        sUnidade = undefined;
      if(oItem){
        sEbeln = oItem.getBindingContext().getProperty("NumProposta")
...
//        sUnidade = this.getRouter().oHashChanger.getHash().substring(15, 19);
    ...
    _showMulti: function (oItem, bInitial) {
      ...
      if(bInitial){
        ...
        this.getRouter().navTo("mCr", {
          numproposta: oItem.getBindingContext().getProperty("NumProposta"),
