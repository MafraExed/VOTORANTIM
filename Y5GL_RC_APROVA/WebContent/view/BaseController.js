...
sap.ui.define([ "sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_rc_portal/model/formatter" ], function(Controller, History, formatter) {
  "use strict";

  return Controller.extend("vsa.y5gl_rc_portal.view.BaseController", {
        ...
        success : function(oData, response) {
          if(oData.results.length > 0){
            if(oData.results[0].EAdm && oData.results[0].EAdm != ""){
              ...
              if (!t._oDialogAdm) {
                t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_rc_portal.view.fragment.adm", t);
...
//        });
    ...
    setComentario : function(sPath, sBanfn, sBnfpo, sUnidade) {
      var t = this;
      var sText = "";
      this.getOwnerComponent().getModel().read("/Comentarios", { // COMENT�RIOS
        async : true,
        filters : [ new sap.ui.model.Filter("Banfn", "EQ", sBanfn),
              new sap.ui.model.Filter("Bnfpo", "EQ", sBnfpo),
        ...
        success : function(oData, response) {
          ...
          t.getOwnerComponent().getModel("global").setProperty("/" + sBnfpo, sText);
      ...
      if(oIconTabBar){
        ...
        oIconTabBar.bindAggregation("items", sPathToAprova, function(index, context) {
          ...
          return new sap.m.IconTabFilter({
            ...
            text : { path: 'NameUsr' },
            content : new sap.m.Text({text: t.getOwnerComponent().getModel("global").getProperty("/" + context.getObject().Bnfpo)})
          });
          }else{
            ...
            var fil = new sap.ui.model.Filter({ filters: [ new sap.ui.model.Filter("IUnidade", "EQ", context.getObject().Unidade), 
                                    new sap.ui.model.Filter("IDocumento", "EQ", context.getObject().Banfn),
                                     new sap.ui.model.Filter("IBukrs", "EQ", context.getObject().Bukrs),
                                     new sap.ui.model.Filter("IBnfpo", "EQ", context.getObject().Bnfpo),
                                     new sap.ui.model.Filter("IProcesso", "EQ", "RC"),
