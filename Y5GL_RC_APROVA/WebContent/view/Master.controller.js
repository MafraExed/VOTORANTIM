...
sap.ui.define([
  ...
  "vsa/y5gl_rc_portal/model/grouper",
  "vsa/y5gl_rc_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_rc_portal.view.Master", {
...
//          }
    ...
    onUserPress: function(oEvent){
      ...
      var sTitle = oObject.Banfn + "/" + oObject.Bnfpo;
        ...
        if (this._oList.getItems().length > 0){
          ...
          if (this.sName === "sRc") {
            ...
            for (var i = 0; i < aItems.length; i++) {
              ...
              if (oObject.Banfn === this.sBanfn && oObject.Unidade === this.sUnidade &&
                oObject.Bnfpo === this.sBnfpo) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.Name1.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.Banfn.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.Bnfpo.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
            ) {
            if(!bFound){
    ...
    onSemanticFilterPress: function(){
         if (!this._oDialogApprove_S) {
           this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_rc_portal.view.fragment.advsearch", this);
  ...
  if(!Device.system.phone){;
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_rc_portal.view.fragment.filter", this);
    ...
    _onMasterMatched: function (oEvent) {
      ...
      this.sBanfn = (oParams && oParams.banfn ? oParams.banfn : "" );
      this.sBnfpo = (oParams && oParams.bnfpo ? oParams.bnfpo : "" );
...
//          }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      var sBanfn,
        ...
        sUnidade = undefined
      if(oItem){
        sBanfn = oItem.getBindingContext().getObject().Banfn;
        sBnfpo = oItem.getBindingContext().getObject().Bnfpo;
        sUnidade = oItem.getBindingContext().getObject().Unidade;
      }else{
        sBanfn = this.getRouter().oHashChanger.getHash().substring(4, 14);
        sUnidade = this.getRouter().oHashChanger.getHash().substring(15, 19);
      }
      this.getRouter().navTo("sRc", {
        banfn: sBanfn,
        bnfpo: sBnfpo,
        unidade: sUnidade
      }, bReplace);
    ...
    _showMulti: function (oItem, bInitial) {
      ...
      if (bInitial){
        ...
        this.getRouter().navTo("mRc", {
          banfn: oItem.getBindingContext().getProperty("Banfn"),
          bnfpo: oItem.getBindingContext().getProperty("Bnfpo"),
