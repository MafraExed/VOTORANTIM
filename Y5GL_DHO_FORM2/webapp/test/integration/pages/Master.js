sap.ui.define([
	...
	"Y5GL_DHO_FORM2/Y5GL_DHO_FORM2/test/integration/pages/Common",
			...
			actions : {
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "ZET_GLHR_FORMULARIOS_BENEFICIOSSet",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Beneficio") === oObject.Beneficio;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Beneficio;
									} else {
										sCurrentId = aItemsNotInTheList[0].Beneficio;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZET_GLHR_FORMULARIOS_BENEFICIOSSet", {
										Beneficio : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Beneficio"),
						title: oBindingContext.getProperty("DescInfotipo")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<ZET_GLHR_FORMULARIOS_BENEFICIOSSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("DescInfotipo");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ZET_GLHR_FORMULARIOS_BENEFICIOSSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<ZET_GLHR_FORMULARIOS_BENEFICIOSSet> (" + iExpectedLength + ")"}),
								success : function () {
