...
sap.ui.define([ "sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_mv_portal/model/formatter" ], function(Controller, History, formatter) {
  "use strict";

  return Controller.extend("vsa.y5gl_mv_portal.view.BaseController", {
        ...
        success : function(oData, response) {
          if(oData.results.length > 0){
            if(oData.results[0].EAdm && oData.results[0].EAdm != ""){
              ...
              if (!t._oDialogAdm) {
                t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_mv_portal.view.fragment.adm", t);
  ...
  setGlobalInterrupt: function(oData){
    ...
    readHeaderFields: function(sUnidade){
      return new Promise(function (success, error) {
          this.getOwnerComponent().getModel().read("/Campos", { // HEADER
              ...
              success : function(oData, response) {
                if(oData.results.length > 0){
                  this.getOwnerComponent().getModel('hFields').setData(oData);
                  success(oData);
                }else{
                  error({
                    message: this.getResourceBundle().getText("CAMPOSDINAMICOS")
                  });
                }
              }.bind(this),
              error: function(oError){
    ...
    setComentario : function(sPath, sDmkey, sUnidade) {
      ...
      this.getOwnerComponent().getModel().read("/Comentarios", { // COMENT�RIOS
        async : true,
        filters : [ new sap.ui.model.Filter("Processo", "EQ", sDmkey),
        ...
        success : function(oData, response) {
          ...
          t.getOwnerComponent().getModel("global").setProperty("/" + sDmkey, sText);
      ...
      if(oIconTabBar){
        ...
        oIconTabBar.bindAggregation("items", sPathToAprova, function(index, context) {
          ...
          return new sap.m.IconTabFilter({
            ...
            text : { path: 'NameUsr' },
            content : new sap.m.Text({text: t.getOwnerComponent().getModel("global").getProperty("/" + context.getObject().Dmkey)})
          });
          }else{
            ...
            var fil = new sap.ui.model.Filter({ filters: [ new sap.ui.model.Filter("IUnidade", "EQ", context.getObject().Unidade),
                         new sap.ui.model.Filter("IDocumento", "EQ", context.getObject().Processo),
                                     ...
                                     new sap.ui.model.Filter("IProcesso", "EQ", "MV"),
