sap.ui.define([
	...
	"use strict";
	var _timeout;
	return Controller.extend("zvpwmm0001_po.controller.Page", {
		...
		createPopover: function() {
			if (!this._oQuickView) {
				this._oQuickView = sap.ui.xmlfragment("zvpwmm0001_po.fragment.Grupo", this);
		...
		onRejeitar: function(oEvent) {
			if (!this._dialog) {
				var vBusyDialog = sap.ui.xmlfragment("zvpwmm0001_po.fragment.BusyDialogRej", this);
		...
		onAprovar: function(oEvent) {
			var that = this;
			if (!this._dialog) {
				var vBusyDialog = sap.ui.xmlfragment("zvpwmm0001_po.fragment.BusyDialog", this);
				this.getView().addDependent(vBusyDialog);
			}
		...
		_onRouteMatched: function(oEvent) {
			...
			oView = this.getView();
			oView.bindElement({
				path: "/Compras(Documento='" + oArgs.Documento + "')",
				events: {
		...
		handleResponsivePopoverPress: function(oEvent) {
			...
			//	if (!this._oPopover) {
			var oView = this.getView();
			this._oPopover = sap.ui.xmlfragment(oView.getId(), "zvpwmm0001_po.fragment.Grupo", this);
		...
		onMessagePopOver: function(oArgs) {
			var that = this;
			var oModel = this.getView().getModel();
			var sPath = ("/Compras(Documento='" + oArgs.Documento + "')/ToLog");
			oModel.read(sPath, {
				success: function(oData, oResponse) {
		...
		onHistory: function(oEvent) {
			...
			var oView = this.getView();
			this._oPopover = sap.ui.xmlfragment(oView.getId(), "zvpwmm0001_po.fragment.History", this);
			this.getView().addDependent(this._oPopover);
			this._oPopover.openBy(oEvent.getSource());
