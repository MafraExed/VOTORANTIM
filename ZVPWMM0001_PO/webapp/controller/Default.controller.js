sap.ui.define([
	...
	'sap/m/MessageBox',
	'zvpwmm0001_po/model/formatter'
], function (jQuery, Controller, Filter, History, JSONModel, MessageBox, formatter) {
	"use strict";

	return Controller.extend("zvpwmm0001_po.controller.Default", {
		formatter: formatter,
		onInit: function () {
			var that = this;
		...
		onApprove: function (oEvent) {
			...
			if (rows.length === 0) {
				...
				jQuery.sap.syncStyleClass("sapUiSizeCompact", this.getView(), vBusyDialog);
				vBusyDialog.open();
				var oModel = new sap.ui.model.odata.ODataModel("/sap/opu/odata/sap/ZGWFBPO_PORTAL_SRV/");
				...
				for (var i = 0; i < rows.length; i++) {
					...
					batchChanges.push(oModel.createBatchOperation("Compras", "POST", oEntry));
				}
