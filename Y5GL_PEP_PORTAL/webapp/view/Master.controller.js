/*global history */
sap.ui.define([
  "vsa/y5gl_pep_portal/view/BaseController",
  "sap/ui/model/json/JSONModel",
  "sap/ui/model/Filter",
  "sap/ui/model/FilterOperator",
  "sap/m/GroupHeaderListItem",
  "sap/ui/Device",
  "vsa/y5gl_pep_portal/model/formatter",
  "vsa/y5gl_pep_portal/model/grouper",
  "vsa/y5gl_pep_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_pep_portal.view.Master", {
    ...
    onUpdateFinished: function (oEvent) {
      ...
      if (oEvent.getParameter("total") > 0 && !Device.system.phone) {
        ...
        if (!this.getOwnerComponent().getModel('global').getProperty("/unidade")) {
          ...
          if (sUnidade) {
            ...
            var oHFields = this.getOwnerComponent().getModel('hFields').getData().results;
            var oDFields = this.getOwnerComponent().getModel('dFields').getData().results;
            if (oHFields && oHFields.length > 0 && oDFields && oDFields.length > 0) {
        ...
        if (this._oList.getItems().length > 0) {
          ...
          if (this.sName === "sPep") {
            ...
            for (var i = 0; i < aItems.length; i++) {
              ...
              if (oObject.Pspid === this.sProcesso && oObject.Unidade === this.sUnidade) {
    ...
    onSearch: function (oEvent) {
      ...
      for (var i = 0; i < aItems.length; i++) {
        ...
        if (sQuery && sQuery != "") {
          if (mItem.Pspid.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
            mItem.Nomeuname.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
          ) {
            if (!bFound) {
              bFound = true;
              bIndex = i;
    ...
    onSemanticFilterPress: function () {
      if (!this._oDialogApprove_S) {
        this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_pep_portal.view.fragment.advsearch", this);
    ...
    onConfirmAdvSearch: function () {
      ...
      for (var i = 0; i < aItems.length; i++) {
        ...
        if (oItemData.ToItem && oItemData.ToItem.__list && oItemData.ToItem.__list.length > 0) {
          var sPath = "/" + oItemData.ToItem.__list[0];
    ...
    onMultiSelectPress: function (e) {
      ...
      if (this._oList.getMode() === "MultiSelect") {
        ...
        if (this.getRouter().oHashChanger.getHash().indexOf("mGa") != -1) {
          this._showDetail(this._oList.getItems()[0]);
        }
      } else {
        this._oList.setMode('MultiSelect');
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_pep_portal.view.fragment.filter", this);
    ...
    _showDetail: function (oItem) {
      ...
      var sProcesso,
          sUnidade = undefined;
      if (oItem) {
        sProcesso = oItem.getBindingContext().getProperty("Pspid");
        sProcesso = sProcesso.replaceAll("/","*");
    ...
    _showMulti: function (oItem, bInitial) {
      ...
      if (bInitial) {
        ...
        this.getRouter().navTo("mPep", {
          processo: oItem.getBindingContext().getProperty("Pspid"),
