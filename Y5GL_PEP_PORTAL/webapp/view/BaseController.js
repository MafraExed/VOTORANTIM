...
sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_pep_portal/model/formatter"], function (Controller,
	History, formatter) {
	"use strict";

	return Controller.extend("vsa.y5gl_pep_portal.view.BaseController", {
				...
				success: function (oData, response) {
					if (oData.results.length > 0) {
						if (oData.results[0].EAdm && oData.results[0].EAdm != "") {
							...
							if (!t._oDialogAdm) {
								t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_pep_portal.view.fragment.adm", t);
					...
					if (context.getObject().Status != "A") {
						return new sap.m.IconTabFilter({
							...
							content: new sap.m.Text({
								text: t.getOwnerComponent().getModel("global").getProperty("/" + context.getObject().Processo)
							})
						});
					} else {
						...
						var fil = new sap.ui.model.Filter({
							filters: [new sap.ui.model.Filter("IUnidade", "EQ", context.getObject().Unidade),
								...
								new sap.ui.model.Filter("IProcesso", "EQ", "PE"),
