sap.ui.define([
	...
	return BaseController.extend("Y5GL_EC_CAD4.Y5GL_EC_CAD4.controller.PESSOA_REFERENCIA", {
		...
		onSave: function () {
			...
			if (oEntry.Name2 === "") {
				sap.m.MessageBox.error("c/o No llenado");
				return;
			}
			
			if (IdRua === "") {
				sap.m.MessageBox.error("Calle y número No llenado");
				return;
			}
			...
			oEntry.Strds = this.getView().byId("IdTipo_End").getSelectedKey();

			var dialog = new Dialog({
				title: "Confirmación",
				type: "Message",
				content: new Text({
					text: "Confirma los cambios de contacto?"
				}),
					...
					press: function () {
						oModel.update(Key, oEntry, {
							success: function (oData, oResponse) {
								sap.m.MessageBox.success("Modificado com suceso.", {
									actions: ["OK"],
