...
sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_ga_portal/model/formatter"], function (Controller,
  History, formatter) {
  "use strict";

  return Controller.extend("vsa.y5gl_ga_portal.view.BaseController", {
        ...
        success: function (oData, response) {
          if (oData.results.length > 0) {
            if (oData.results[0].EAdm && oData.results[0].EAdm != "") {
              ...
              if (!t._oDialogAdm) {
                t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_ga_portal.view.fragment.adm", t);
    ...
    setComentario: function (sPath, sDmkey, sUnidade) {
      var t = this;
      var sText = "";
      return new Promise(function (fnResolve, fnReject) {
        this.getOwnerComponent().getModel().read("/Comentarios", { // COMENT�RIOS
          async: true,
          filters: [new sap.ui.model.Filter("Dmkey", "EQ", sDmkey),
          ...
          if (context.getObject().Status != "A") {
            return new sap.m.IconTabFilter({
              ...
              content: new sap.m.Text({
                text: t.getOwnerComponent().getModel("global").getProperty("/comentario")
              })
            });
          } else {
            ...
            var fil = new sap.ui.model.Filter({
              filters: [new sap.ui.model.Filter("IUnidade", "EQ", context.getObject().Unidade),
                new sap.ui.model.Filter("IDocumento", "EQ", context.getObject().Dmkey),
                //Linhas comentadas em 22/07/2021        // new sap.ui.model.Filter("IBukrs", "EQ", context.getObject().Bukrs),
                // por T3RODRIGODC                       // new sap.ui.model.Filter("IGjahr", "EQ", context.getObject().Gjahr),
                new sap.ui.model.Filter("IProcesso", "EQ", "GA"),
    ...
    onDmkeyPress: function(oEvent) {
      var oObject = oEvent.getSource().getParent().getBindingContext().getObject();
      if (oObject) {
        var sBukrs = oObject.BukrsOrigem;
        var sDmkey = oObject.Dmkey;

        //Monta a URL da ZT502FMI
        var sUrlZT502FMI = "sap/bc/gui/sap/its/webgui/its/webgui?~transaction=*ZT502FMI+";
        sUrlZT502FMI += "s_bukrs-low=" + sBukrs + ";";
        sUrlZT502FMI += "s_dmkey-low=" + sDmkey;

        //Navega para a URL
        sap.m.URLHelper.redirect(this.getMainUrl() + sUrlZT502FMI, true);
        }

     },

     //FIM - T3RODRIGODC - 08/06/2022
