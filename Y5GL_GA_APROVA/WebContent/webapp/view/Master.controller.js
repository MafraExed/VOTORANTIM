...
sap.ui.define([
  ...
  "vsa/y5gl_ga_portal/model/grouper",
  "vsa/y5gl_ga_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_ga_portal.view.Master", {
...
//          }
        ...
        if (this._oList.getItems().length > 0){
          ...
          if (this.sName === "sGa") {
            ...
            for (var i = 0; i < aItems.length; i++) {
              ...
              if (oObject.Dmkey === this.sDmkey && oObject.Unidade === this.sUnidade) {
    ...
    onSearch: function (oEvent) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.IdProcFmi.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
              mItem.Name1Origem.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
            ) {
                if(!bFound){
    ...
    onSemanticFilterPress: function(){
         if (!this._oDialogApprove_S) {
           this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_ga_portal.view.fragment.advsearch", this);
  ...
  if(!Device.system.phone){;
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_ga_portal.view.fragment.filter", this);
    ...
    _onMasterMatched: function (oEvent) {
      ...
      this.sDmkey = (oParams && oParams.dmkey ? oParams.dmkey : "" );
...
//          }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      var sDmkey,
        sUnidade = undefined;
      if(oItem){
        sDmkey   = oItem.getBindingContext().getProperty("Dmkey");
...
//        sUnidade = oItem.getBindingContext().getProperty("Unidade")
//        sDmkey = this.getRouter().oHashChanger.getHash().substring(4, 14);
//        sUnidade = this.getRouter().oHashChanger.getHash().substring(15, 19);
      }
      this.getRouter().navTo("sGa", {
        dmkey : sDmkey,
        unidade: sUnidade
      }, bReplace);
    },
    ...
    _showMulti: function (oItem, bInitial) {
      ...
      if (bInitial){
        ...
        this.getRouter().navTo("mGa", {
          dmkey   : oItem.getBindingContext().getProperty("Dmkey"),
