 ...
 */
sap.ui.define(['./lib/comboBox', './lib/dateRangeSelection', './lib/datePicker', './lib/input', './lib/multiComboBox', './lib/multiInput', './lib/stepInput', './lib/switch', './lib/vBox', 'com/innova/util/isFloatNumber'], function (comboBox, dateRangeSelection, datePicker, input, multiComboBox, multiInput, stepInput, vSwitch, vBox, isFloatNumber) {
  var formUtils = _objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread({}, comboBox), dateRangeSelection), datePicker), input), multiComboBox), multiInput), stepInput), vSwitch), vBox), {}, {
    /* =========================================================== */
    ...
    /**
     * @function
     * @name onChangeNumberP
     * @description - Se ejecuta en el evento change es llamado
     *
     * @private
     ...
     * @version 0.5.0
     */
    onChangeNumberP: function onChangeNumberP(oCatalog, oEvent) {
      var oSource = oEvent.getSource();
      var oValueContext = oSource.getBinding('value');
      var oContext = oSource.getParent().getBindingContext();
      var oNumberFormat = {}.getFloatFormat({
        decimals: parseInt(oCatalog.Decimals, 10),
        maxIntegerDigits: parseInt(oCatalog.INTLEN, 10)
      });
      var sValue = formUtils.getInputValue(oSource);
      var fValue = oNumberFormat.parse(sValue);
      fValue = oNumberFormat.format(fValue);
      fValue = oNumberFormat.parse(fValue);
      var oModel = oContext.getModel();
      var sPath = "".concat(oContext.getPath());
      oModel.setProperty("".concat(sPath, "/selected"), true);
      oModel.setProperty("".concat(sPath, "/updated"), true);
      oModel.setProperty("".concat(sPath, "/").concat(oValueContext.getPath()), !Number.isNaN(fValue) ? fValue : 0.0);
    },
    ...
    validateForm: function validateForm(_ref) {
      ...
      formContainers.map(formUtils._mapFields).filter(formUtils._isRequiredField).forEach(function (field) {
        ...
        var isValid = formUtils[strategy]({
          field: field
        });
    ...
    setDataInFields: function setDataInFields(_ref3) {
      ...
      formElements.forEach(function (element) {
        ...
        if (name === data.FIELDNAME) {
          var className = field.getMetadata().getElementName();
          var nameField = className.split('.').pop();
