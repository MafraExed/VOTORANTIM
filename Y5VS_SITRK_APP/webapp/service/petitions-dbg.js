...
sap.ui.define(['./service', './strategies', './utils/buildData', './utils/buildRes', 'com/innova/vendor/lodash.get'], function (service, strategies, buildData, buildRes, get) {
  ...
  var petitions = {
    ...
    returnData: function returnData(_ref) {
      ...
      return {
        ...
        success: function success(data, response) {
          try {
            var results = get(data, 'ZAT_VSMM_ASSPOSTDATA.results', []);
    ...
    find: function find(path, data) {
      ...
      var req = {
        Mandt: '100',
        ZAT_VSMM_ASSPOSTDATA: [strategies.post["".concat(path)]].concat(_toConsumableArray(results))
      };
      return model.metadataLoaded().then(function () {
        return new Promise(function (resolve, reject) {
          model.create('/ET_VSMM_FIORISet', req, petitions.returnData({
            config: config,
            data: data,
            options: options,
            reject: reject,
    ...
    get: function get(path, config) {
      var req = {
        Mandt: '100',
        ZAT_VSMM_ASSPOSTDATA: [strategies.get["".concat(path)]]
      };
      return model.metadataLoaded().then(function () {
        return new Promise(function (resolve, reject) {
          model.create('/ET_VSMM_FIORISet', req, petitions.returnData({
    ...
    update: function update(path, data, config) {
      ...
      var req = {
        Mandt: '100',
        ZAT_VSMM_ASSPOSTDATA: [strategies.put["".concat(path)]].concat(_toConsumableArray(results))
      };
      return model.metadataLoaded().then(function () {
        return new Promise(function (resolve, reject) {
          model.create('/ET_VSMM_FIORISet', req, petitions.returnData({
