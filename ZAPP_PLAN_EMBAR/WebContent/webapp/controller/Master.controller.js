...
sap.ui.define([
		...
		return BaseController.extend("nasa.ui5.planejamentoEmbarque.controller.Master", {
			...
			onSearch : function (oEvent) {
				...
				if (sQuery){
					...
					//Clear Filter except Status and Date(Quick Search)
					this._oListFilterState.aSearch = this._oListFilterState.aSearch.filter(function(item) { 
						return item.sPath == "Status" || item.sPath == "Dteta";
					});
					
					if(!this._oListFilterState)
			...
			onHandleConfirmStatusFilter: function(oEvent){
				...
				//Clear Filter except Dcrnv and Date(Quick Search)
				this._oListFilterState.aSearch = this._oListFilterState.aSearch.filter(function(item) { 
					return item.sPath == "Dcrnv" || item.sPath == "Dteta";
				});
				
				var oStatusItem = oEvent.getSource().getSelectedItem();
	 
				if(!!oStatusItem && oStatusItem.getKey() !== constant.ALL_STATUS) {			
			...
			_createViewModel : function() {
				return new JSONModel({
					...
					sortBy: "Dcrnv",
					groupBy: "None",
					statusFilter: [{ Status: constant.PENDENT_STATUS },
					               { Status: constant.START_STATUS },
					               { Status: constant.COMPLETED_STATUS },
					               { Status: constant.ALL_STATUS }]
				});
			},
			...
			_onMasterMatched :  function() {
				this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
						function (mParams) {
							...
							/*var sDcrnv = mParams.firstListitem.getBindingContext().getProperty("Dcrnv");
							var sNvoyg = mParams.firstListitem.getBindingContext().getProperty("Nvoyg");
							//var sPtorg = mParams.firstListitem.getBindingContext().getProperty("Ptorg");
										
							this.getRouter().navTo("object", {	Dcrnv: sDcrnv,
																Nvoyg: sNvoyg}, true);*/
																//Ptorg: sPtorg}, true);
								}.bind(this),
						  function (mParams) {
			...
			_showDetail : function (oItem) {
				...
				this.getRouter().navTo("object", {
					Dcrnv : oItem.getBindingContext().getProperty("Dcrnv"),
					Nvoyg : oItem.getBindingContext().getProperty("Nvoyg")
					//Ptorg : oItem.getBindingContext().getProperty("Ptorg")
