...
sap.ui.define([
  ...
  "vsa/y5gl_iv_portal/model/grouper",
  "vsa/y5gl_iv_portal/model/GroupSortState"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, Formatter, grouper, GroupSortState) {
  "use strict";

  return BaseController.extend("vsa.y5gl_iv_portal.view.Master", { 
...
//          }
    ...
    if (this._oList.getItems().length > 0){
      ...
      if (this.sName === "sIv") {
        ...
        for (var i = 0; i < aItems.length; i++) {
          ...
          if (oObject.Ivkey === this.sIvkey && oObject.Unidade === this.sUnidade) {
    ...
    onSearch: function (oEvent) {
      ...
      for(var i=0; i<aItems.length; i++){
        ...
        if (sQuery && sQuery != "") {
          if (  mItem.Iblnr.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
            mItem.Gjahr.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1 ||
            mItem.Nomewerks.toUpperCase().indexOf(sQuery.toUpperCase()) !== -1
            ) {
            if(!bFound){
    ...
    onSemanticFilterPress: function(){
       if (!this._oDialogApprove_S) {
         this._oDialogApprove_S = sap.ui.xmlfragment("AdvancedSearchDialog", "vsa.y5gl_iv_portal.view.fragment.advsearch", this);
  ...
  if(!Device.system.phone){;
    ...
    onOpenViewSettings: function () {
      if (!this._oViewSettingsDialog) {
        this._oViewSettingsDialog = sap.ui.xmlfragment("vsa.y5gl_iv_portal.view.fragment.filter", this);
    ...
    this.sIvkey = (oParams && oParams.ivkey ? oParams.ivkey : "" );
...
//        }.bind(this)));
    ...
    _showDetail: function (oItem) {
      ...
      var sEbeln,
        sUnidade = undefined;
      if(oItem){
        sEbeln = oItem.getBindingContext().getProperty("Ivkey")
    ...
    _showMulti: function (oItem, bInitial) {
      ...
      if (bInitial){
          ...
          this.getRouter().navTo("mIv", {
              ivkey: oItem.getBindingContext().getProperty("Ivkey"),
