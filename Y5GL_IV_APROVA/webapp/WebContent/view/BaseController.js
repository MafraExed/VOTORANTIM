...
sap.ui.define([ "sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_iv_portal/model/formatter" ], function(Controller, History, formatter) {
	"use strict";

	return Controller.extend("vsa.y5gl_iv_portal.view.BaseController", {
				...
				success : function(oData, response) {
					if(oData.results.length > 0){
						if(oData.results[0].EAdm && oData.results[0].EAdm != ""){
							...
							if (!t._oDialogAdm) {
								t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_iv_portal.view.fragment.adm", t);
...
//				});
		...
		setComentario : function(sPath, sEbeln, sUnidade) {
			...
			return new Promise(function (fnResolve, fnReject) {
				this.getOwnerComponent().getModel().read("/Comentarios", { // COMENT?RIOS
					async : true,
					filters : [ new sap.ui.model.Filter("Ivkey", "EQ", sEbeln),
		...
		onAcao: function (sModo, aTab, sComent, sRejectionText) {
			...
			var sRellevel = this.getOwnerComponent().getModel('global').getProperty("/rellevel");
