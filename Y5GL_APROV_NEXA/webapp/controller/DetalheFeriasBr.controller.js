sap.ui.define(["Y5GL_APROV_NEXA/Y5GL_APROV_NEXA/controller/BaseController", "sap/ui/model/json/JSONModel",
	"Y5GL_APROV_NEXA/Y5GL_APROV_NEXA/model/formatter", "sap/ui/Device", "sap/m/Dialog", "sap/m/Button", "sap/m/Text", "sap/ui/core/Fragment",
	...
	return e.extend("Y5GL_APROV_NEXA.Y5GL_APROV_NEXA.controller.DetalheFeriasBr", {
		formatter: a,
		...
		onInit: function () {
			...
			this.getRouter().getRoute("DetalheFeriasBr").attachPatternMatched(this._onObjectMatched, this);
			this.setModel(e, "detailView");
			this.getOwnerComponent().getModel().metadataLoaded().then(this._onMetadataLoaded.bind(this));
			var a = jQuery.sap.getModulePath("Y5GL_APROV_NEXA.Y5GL_APROV_NEXA");
		...
		somardatafirst: function () {
			...
			if (i === true) {
				...
				sap.m.MessageBox.error("El valor informado debe ser un número.", {
					actions: ["OK"],
					onClose: function (t) {
		...
		somardata: function (e, t) {
			...
			i.read(o, {
				...
				error: function () {
					sap.m.MessageToast.show("Vacaciones no encontradas!")
				}
			})
		},
		...
		AbreDialogSave: function () {
			...
			if (!e._valueHelpDialog1) {
				e._valueHelpDialog1 = sap.ui.xmlfragment("Y5GL_APROV_NEXA.Y5GL_APROV_NEXA.view.dialog", e);
		...
		AbreDialogCancel: function () {
			...
			if (!e._valueHelpDialog2) {
				e._valueHelpDialog2 = sap.ui.xmlfragment("Y5GL_APROV_NEXA.Y5GL_APROV_NEXA.view.dialogCancel", e);
		...
		onSelect: function (e) {
			...
			var l = n[1];
			var g = "La solicitud del decimotercer salario será posible solo una vez!";
			if (l === "first_13_1") {
		...
		onConfirmaDialog: function () {
			...
			if (e !== true && t !== true && a !== true) {
				sap.m.MessageBox.error("Debe seleccionarse al menos un período");
				return
			}
			this.onSave(e, t, a)
		},
		...
		onCancel: function (e, t, a) {
			...
			if (V === 1) {
				sap.m.MessageBox.error("Hay información sin completar.", {
					actions: ["OK"],
					onClose: function (e) {
			...
			var w = new s({
				...
				type: "Message",
				content: new r({
					text: "¿Quieres cancelar tu solicitud de vacaciones?"
				}),
				beginButton: new o({
					text: "Si",
					press: function () {
						...
						b.update(l, g, {
							success: function (e, t) {
								sap.m.MessageBox.success("Tus vacaciones se cancelaron con éxito.", {
									actions: ["OK"],
									onClose: function (e) {
			...
			var f = new s({
				...
				beginButton: new o({
					...
					press: function () {
						...
						b.update(l, g, {
							success: function (e, t) {
								sap.m.MessageBox.success("Sus vacaciones se han programado correctamente.", {
									actions: ["OK"],
									onClose: function (e) {
										i.getView().getModel().refresh(true)
									}
		...
		onAprova: function (oEvent) {
			...
			var split = id.split("DetalheFeriasBr--");
			var idnew = split[1];
			var acao = "A";
		...
		onReprova: function (oEvent) {
			...
			var dialog = "";
			dialog = new Dialog({
				title: "Motivo de la desaprobación",
				type: "Message",
				content: [
					new Label({
						text: "Describe el motivo del rechazo:",
		...
		save: function (e, t, a, periodo, acao) {
			...
			if (acao === "A") {
				messagem_erro = "No hay información para aprobar.";
				messagem_pergunta = "¿Confirmar la aprobación de vacaciones?";
				messagem_sucesso = "Aprobación exitosa!";
				msg_pendente =
					"¡Vacaciones aprobadas con éxito! \ n \ n Este empleado tiene otros períodos pendientes de aprobación, ¿desea continuar?";
			}

			if (acao === "R") {
				messagem_erro = "No hay información para desaprobar.";
				messagem_pergunta = "¿Confirma su desaprobación de vacaciones?";
				messagem_sucesso = "Desaprobación realizado con éxito!";
				msg_pendente =
					"¡Vacaciones rechazadas correctamente! \ n \ n Este empleado tiene otros períodos pendientes de aprobación, ¿desea continuar?";
			}

			pendente = "En aprovacion";
