...
// jQuery.sap.require("vsa.y5gl_rl_aprova.base");
sap.ui.define([ "sap/ui/core/mvc/Controller", "sap/ui/core/routing/History", "vsa/y5gl_po_portal/model/formatter" ], 
function(Controller, History, formatter) { "use strict";

  return Controller.extend("vsa.y5gl_po_portal.view.BaseController", {
        ...
        success : function(oData, response) {
          if(oData.results.length > 0){
            if(oData.results[0].EAdm && oData.results[0].EAdm != ""){
              ...
              if (!t._oDialogAdm) {
                t._oDialogAdm = sap.ui.xmlfragment("vsa.y5gl_po_portal.view.fragment.adm", t);
    ...
    setComentario : function(sPath, sEbeln, sUnidade) {
      ...
      this.getOwnerComponent().getModel().read("/Comentarios", { // COMENTÁRIOS
        ...
        success : function(oData, response) {
          ...
          t.getOwnerComponent().getModel("global").setProperty("/" + sEbeln, sText);
      ...
      if(oIconTabBar){
        ...
        oIconTabBar.bindAggregation("items", sPathToAprova, function(index, context) {
          ...
          return new sap.m.IconTabFilter({
            ...
            text : { path: 'NameUsr' },
            content : new sap.m.Text({text: t.getOwnerComponent().getModel("global").getProperty("/" + context.getObject().Ebeln)})
          });
          }else{
            ...
            var fil = new sap.ui.model.Filter({ filters: [ new sap.ui.model.Filter("IUnidade", "EQ", context.getObject().Unidade),
                                     new sap.ui.model.Filter("IDocumento", "EQ", context.getObject().Ebeln),
                                     new sap.ui.model.Filter("IProcesso", "EQ", "PO"),
...
// content: [
// //new sap.m.Label({ text: this.getResourceBundle().getText("REJECTTEXT"),
...
// oVBox.addItem(new sap.m.Label({ text:
// this.getResourceBundle().getText("MAILTO"), labelFor: 'txtMailTo'}),
// new sap.m.Text('txtMailTo', { text : sContext.NameText } ));
// oVBox.addItem(new sap.m.Label({ text:
// this.getResourceBundle().getText("MAILAD"), labelFor: 'txtMailAd'}));
// oVBox.addItem(new sap.m.Text('txtMailAd', { text : sContext.EMail } ));
// oVBox.addItem(new sap.m.Label({ text:
// this.getResourceBundle().getText("MAILBODY"), labelFor: 'txtAreaQuestion'}));
