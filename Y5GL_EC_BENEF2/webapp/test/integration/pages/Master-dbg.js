			...
			actions : {
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "ZET_GLHR_MEU_CADASTROSet",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Programm") === oObject.Programm;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Programm;
									} else {
										sCurrentId = aItemsNotInTheList[0].Programm;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZET_GLHR_MEU_CADASTROSet", {
										Programm : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Programm"),
						title: oBindingContext.getProperty("Zdesc")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<ZET_GLHR_MEU_CADASTROSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Zdesc");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ZET_GLHR_MEU_CADASTROSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<ZET_GLHR_MEU_CADASTROSet> (" + iExpectedLength + ")"}),
								success : function () {
